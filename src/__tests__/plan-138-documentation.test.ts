...
describe('Plan 138 Documentation', () => {
  ...
  describe('CHANGELOG.md', () => {
    it('should contain compliance audit remediation section', () => {
      const changelogPath = join(rootDir, 'CHANGELOG.md');
      const content = readFileSync(changelogPath, 'utf-8');

      expect(content).toContain('Compliance Audit Remediation (Plan 138)');
      expect(content).toContain('AI vendor disclosure');
    });

    it('should document Anthropic Claude and Azure Voice disclosure', () => {
      const changelogPath = join(rootDir, 'CHANGELOG.md');
      const content = readFileSync(changelogPath, 'utf-8');

      expect(content).toContain('Anthropic Claude');
      expect(content).toContain('Azure Realtime Voice API');
    });
    ...
    it('should document cookie documentation expansion (9 cookies)', () => {
      ...
      expect(content).toContain('security attributes');
    });
