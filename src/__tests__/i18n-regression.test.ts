...
describe('i18n Regression Tests', () => {
    ...
    it('should have all 5 language directories with namespace files', () => {
      SUPPORTED_LOCALES.forEach((locale) => {
        ...
        NAMESPACES.forEach((ns) => {
          ...
          expect(fs.existsSync(nsPath), `Missing ${locale}/${ns}.json`).toBe(true);
        });
      });
    });
    ...
    it('should have required namespaces in all languages', () => {
      ...
      SUPPORTED_LOCALES.forEach((locale) => {
        ...
        requiredNamespaces.forEach((ns) => {
          expect(messages[ns], `${locale}.json missing namespace: ${ns}`).toBeDefined();
        });
      });
    });
    ...
    it('should have consistent variable placeholders', () => {
      ...
      SUPPORTED_LOCALES.filter((l) => l !== 'it').forEach((locale) => {
        ...
        keysWithVars.forEach((key) => {
          ...
          const itVars = [...itValue.matchAll(variablePattern)].map((m) => m[1]);
          const localeVars = [...localeValue.matchAll(variablePattern)].map((m) => m[1]);

          // Variables should match (same names, case-sensitive)
          expect(localeVars.sort(), `${locale}.json key "${key}" has different variables`).toEqual(
            itVars.sort(),
          );
        });
      });
  ...
  describe('2. Formality Rules (ADR 0064)', () => {
    it('should classify historical professors as formal', () => {
      FORMAL_PROFESSORS.forEach((prof) => {
        ...
        expect(FORMAL_PROFESSORS.includes(prof), `${prof} should be in FORMAL_PROFESSORS`).toBe(
          true,
        );
      });
    });
    ...
    it('should have language-specific formal greetings', () => {
      ...
      const itFormal = getNestedValue(itMessages, 'maestri.greetings.formal') as string;
      if (itFormal) {
        expect(itFormal).toMatch(/Lei|esserLe/i);
      }

      // German formal uses "Sie"
      const deFormal = getNestedValue(deMessages, 'maestri.greetings.formal') as string;
      if (deFormal) {
        expect(deFormal).toMatch(/Sie|Ihnen/i);
      }

      // French formal uses "vous"
      const frFormal = getNestedValue(frMessages, 'maestri.greetings.formal') as string;
      if (frFormal) {
  ...
  describe('3. Language Maestri', () => {
    ...
    it('should have maestri data files', () => {
      ...
      Object.values(LANGUAGE_MAESTRI).forEach((maestro) => {
        ...
        expect(fs.existsSync(knowledgeFile), `Missing ${maestro}-knowledge.ts`).toBe(true);
      });
    });
  });
    ...
    it('should have all critical keys in every language', () => {
      SUPPORTED_LOCALES.forEach((locale) => {
        ...
        criticalKeys.forEach((key) => {
          ...
          expect(value, `${locale}.json missing critical key: ${key}`).toBeDefined();
          expect(
            typeof value === 'string',
            `${locale}.json key ${key} must be a string, got ${typeof value}`,
          ).toBe(true);
          expect((value as string).length > 0, `${locale}.json has empty value for: ${key}`).toBe(
            true,
          );
        });
      });
  ...
  describe('5. 404 Page Translations', () => {
    it('should have 404 page translations in all languages', () => {
      SUPPORTED_LOCALES.forEach((locale) => {
        const messages = loadMessageFile(locale);
        const notFoundPage = getNestedValue(messages, 'errors.notFoundPage') as Record<
          string,
          string
        >;

        expect(notFoundPage, `${locale}.json missing errors.notFoundPage`).toBeDefined();
        expect(notFoundPage.title, `${locale} missing 404 title`).toBeDefined();
        expect(notFoundPage.description, `${locale} missing 404 description`).toBeDefined();
        expect(notFoundPage.backHome, `${locale} missing 404 backHome`).toBeDefined();
      });
    });
  });
  ...
  describe('7. Tool Translations', () => {
    ...
    it('should have tool labels in all languages', () => {
      SUPPORTED_LOCALES.forEach((locale) => {
        ...
        tools.forEach((tool) => {
          ...
          expect(label, `${locale}.json missing tools.${tool}.label`).toBeDefined();
        });
      });
    });
    ...
    it('should have tool descriptions in all languages', () => {
      SUPPORTED_LOCALES.forEach((locale) => {
        ...
        tools.forEach((tool) => {
          const description = getNestedValue(messages, `tools.${tool}.description`);
          expect(description, `${locale}.json missing tools.${tool}.description`).toBeDefined();
        });
      });
    });
  });
  ...
  describe('8. Welcome/Onboarding Translations', () => {
    it('should have welcome namespace in all languages', () => {
      SUPPORTED_LOCALES.forEach((locale) => {
        const messages = loadMessageFile(locale);
        expect(messages.welcome, `${locale}.json missing welcome namespace`).toBeDefined();
      });
    });
    ...
    it('should have welcome hero translations', () => {
      SUPPORTED_LOCALES.forEach((locale) => {
        ...
        expect(welcomeNew, `${locale}.json missing welcome.hero.welcomeNew`).toBeDefined();
      });
    });
  });
  ...
  describe('9. Chat Translations', () => {
    it('should have chat namespace in all languages', () => {
      SUPPORTED_LOCALES.forEach((locale) => {
        const messages = loadMessageFile(locale);
        expect(messages.chat, `${locale}.json missing chat namespace`).toBeDefined();
      });
    });
    ...
    it('should have chat input translations', () => {
      SUPPORTED_LOCALES.forEach((locale) => {
        const messages = loadMessageFile(locale);
        const chatPlaceholder = getNestedValue(messages, 'chat.input.placeholder');
        expect(chatPlaceholder, `${locale}.json missing chat.input.placeholder`).toBeDefined();
      });
    });
  });
