...
function hasOllamaProvider(): boolean {
  return !!(process.env.OLLAMA_URL || process.env.NEXT_PUBLIC_OLLAMA_ENABLED === 'true');
}
...
export default function proxy(request: NextRequest) {
  ...
  if (!isAuthenticated && !hasTrialSession) {
    ...
    const welcomeUrl = new URL(localePrefix ? `/${localePrefix}/welcome` : '/welcome', request.url);
