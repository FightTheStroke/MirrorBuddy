...
export function HeaderVariantA(props: HeaderVariantProps) {
  ...
  return (
    <div
      ...
      style={{ background: `linear-gradient(135deg, ${maestro.color}, ${maestro.color}cc)` }}
    >
      {/* Left: Avatar */}
      <motion.div
      ...
      >
        <Image
          ...
          className={cn(
            ...
            isConnected ? 'border-white shadow-xl shadow-white/20' : 'border-white/40'
          )}
        />
        <span className={cn(
      ...
      <div className="flex-1 flex flex-col gap-2">
        <div className="flex items-center gap-2">
          <h2 className="text-lg sm:text-xl font-bold">{maestro.name}</h2>
        ...
        <div className="flex items-center gap-1.5 h-8 mt-1">
          {VISUALIZER_BARS.map((offset, i) => {
            ...
            const style = !isVoiceActive || !isConnected
              ...
              : isSpeaking
                ? { height: base + outputLevel * variance * 24, opacity: 0.5 + outputLevel * 0.5 }
                : isListening && !isMuted
                  ? { height: base + inputLevel * variance * 28, opacity: 0.4 + inputLevel * 0.6 }
