...
export function ToolContent({ toolCall, sessionId }: ToolContentProps) {
  ...
  if (isLoading) {
    return (
      <div className="h-20 flex items-center justify-center bg-slate-800/50 rounded-xl border border-slate-700">
        <Loader2 className="w-6 h-6 text-blue-500 animate-spin" />
        <span className="ml-2 text-sm text-slate-400">Caricamento materiale...</span>
      </div>
    );
  }
  ...
  switch (toolType) {
    ...
    case 'diagram':
      return <DiagramRenderer request={toolData as unknown as DiagramRequest} />;

    case 'formula':
      return <FormulaRenderer request={toolData as unknown as FormulaRequest} />;

    case 'quiz':
      return <AutoSaveQuiz request={toolData as unknown as QuizRequest} toolId={toolCall.id} />;

    case 'flashcard':
      return <AutoSaveFlashcard request={toolData as unknown as FlashcardDeckRequest} toolId={toolCall.id} />;
    ...
    case 'summary':
      return <AutoSaveSummary request={toolData as unknown as SummaryData} toolId={toolCall.id} />;

    case 'demo':
      return <AutoSaveDemo request={toolData as unknown as DemoData} toolId={toolCall.id} />;
