...
export function InlineComment({ comment, highlightedText, onResolve, onDelete }: InlineCommentProps) {
        ...
        {isOpen && (
          ...
          >
            <div className="px-3 py-2 bg-slate-50 dark:bg-slate-700/50 border-b border-slate-200 dark:border-slate-600 flex items-center gap-2">
              ...
              <span className="text-xs font-medium text-slate-600 dark:text-slate-300">Feedback del Maestro</span>
            </div>
            ...
            {!comment.resolved && (onResolve || onDelete) && (
              <div className="px-3 py-2 bg-slate-50 dark:bg-slate-700/50 border-t border-slate-200 dark:border-slate-600 flex gap-2">
                {onResolve && (
                  <Button size="sm" variant="outline" onClick={() => { onResolve(comment.id); setIsOpen(false); }} className="flex-1 h-7 text-xs">
                    <Check className="w-3 h-3 mr-1" />Risolto
                  </Button>
                )}
                {onDelete && (
                  <Button size="sm" variant="ghost" onClick={() => { onDelete(comment.id); setIsOpen(false); }} className="h-7 text-xs text-red-500">
                    <X className="w-3 h-3" />
                  </Button>
                )}
              </div>
            ...
            {comment.resolved && (
              <div className="px-3 py-2 bg-green-50 dark:bg-green-900/20 border-t border-green-200 dark:border-green-800 flex items-center gap-1.5 text-xs text-green-600">
                <Check className="w-3 h-3" />Hai affrontato questo punto
              </div>
            )}
          </motion.div>
        )}
...
export function TextWithComments({ content, comments, onResolveComment, onDeleteComment }: TextWithCommentsProps) {
  if (!comments.length) return <span>{content}</span>;

  const sorted = [...comments].sort((a, b) => (a.resolved !== b.resolved ? (a.resolved ? 1 : -1) : a.startOffset - b.startOffset));
  const segments: { text: string; comment?: InlineCommentType }[] = [];
  let lastEnd = 0;

  for (const c of sorted) {
    if (c.startOffset >= content.length || c.endOffset > content.length) continue;
    if (c.startOffset > lastEnd) segments.push({ text: content.slice(lastEnd, c.startOffset) });
    if (c.startOffset >= lastEnd) {
      segments.push({ text: content.slice(c.startOffset, c.endOffset), comment: c });
      lastEnd = c.endOffset;
    }
  }
  ...
  return (
    <span>
      {segments.map((s, i) => s.comment
        ? <InlineComment key={s.comment.id} comment={s.comment} highlightedText={s.text} onResolve={onResolveComment} onDelete={onDeleteComment} />
