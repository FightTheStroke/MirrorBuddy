...
export function GuidedSection({
  ...
  return (
    <div
      className={cn(
        "border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden bg-white dark:bg-slate-900",
        className,
      )}
    >
      <button
        ...
        <div className="p-4 space-y-4">
            ...
            {viewMode === "edit" && !readOnly ? (
              ...
              <div className="min-h-[200px] p-4 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 prose prose-sm dark:prose-invert max-w-none">
                {section.content ? (
                  ...
                  <p className="text-slate-400 italic">{t("noContentYet")}</p>
                )}
              </div>
            )}
          </div>
          ...
          {section.comments.length > 0 && viewMode === "edit" && (
            <div className="text-xs text-slate-500 flex items-center gap-2">
              <MessageCircle className="w-3 h-3" />
              {unresolvedCount > 0
                ? `${unresolvedCount} commenti da leggere (vedi anteprima)`
  ...
  return (
    ...
    >
      ...
      <div className="text-xs text-slate-500">
        {wordCount ? `${wordCount} parole` : "Da completare"}
      </div>
    </button>
  );
}
