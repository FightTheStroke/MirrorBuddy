...
const KEY_FINGER_MAP: Record<string, 'left' | 'right'> = {
...
const KEY_FINGER_DETAILED: Record<string, { hand: 'left' | 'right'; finger: keyof typeof FINGER_COLORS.left }> = {
  ...
  i: { hand: 'right', finger: 'middle' }, o: { hand: 'right', finger: 'ring' }, p: { hand: 'right', finger: 'pinky' },
  ...
  v: { hand: 'left', finger: 'index' }, b: { hand: 'right', finger: 'index' },
...
export function FingerPlacement({
  ...
  return (
    ...
    >
      {showHands && (
        <HandsVisualization activeFinger={fingerInfo} activeColor={activeColor} />
      )}
      {currentKey && (
...
interface HandsVisualizationProps {
  activeFinger?: { hand: 'left' | 'right'; finger: keyof typeof FINGER_COLORS.left };
  activeColor?: string | null;
}

function HandsVisualization({ activeFinger, activeColor }: HandsVisualizationProps) {
  return (
    <div className="flex gap-4 items-center">
      <HandSide side="left" activeFinger={activeFinger} activeColor={activeColor} />
      <div className="text-muted-foreground text-sm">or</div>
      <HandSide side="right" activeFinger={activeFinger} activeColor={activeColor} />
    </div>
  );
}
  ...
  return (
    <div className={cn(
      ...
      {Object.entries(colors).map(([finger, color]) => {
        ...
        const backgroundColor = isFingerActive && activeColor ? activeColor : color;
