...
export function AccuracyGame({ onGameEnd }: AccuracyGameProps) {
  ...
  return (
    <div className="space-y-6 max-w-2xl mx-auto">
      <h3 className="text-xl font-bold">{t("accuracyGame")}</h3>

      <p className="text-sm text-muted-foreground">
        {t("digitaLeFrasiCorrettamentePerGuadagnarePunti")}
      </p>

      {isComplete ? (
        <div className="p-8 bg-primary/10 border border-primary/20 rounded-lg text-center">
          <h4 className="text-2xl font-bold mb-2">{t("giocoCompletato")}</h4>
          <p className="text-lg">{t("punteggio")} {score}</p>
        </div>
      ) : (
