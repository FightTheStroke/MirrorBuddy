...
export function TosAcceptanceModal({
  ...
  const t = useTranslations('consent.terms');
  ...
  const handleAccept = async () => {
    ...
    try {
      ...
      logger.error('Failed to save ToS acceptance', { component: 'TosAcceptanceModal' }, error);
        ...
        >
          ...
          <div className="flex flex-col space-y-2 text-center sm:text-left">
            <DialogPrimitive.Title className="text-2xl font-bold leading-none tracking-tight text-slate-900 dark:text-slate-50">
              {isReconsent ? t('modal.title.updated') : t('modal.title.welcome')}
            </DialogPrimitive.Title>
            <DialogPrimitive.Description
              id="tos-description"
              className="text-sm text-slate-600 dark:text-slate-400"
            >
              {isReconsent ? t('modal.description.updated') : t('modal.description.welcome')}
            </DialogPrimitive.Description>
          </div>
          ...
          {isReconsent && (
            <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
              <p className="text-sm text-blue-800 dark:text-blue-200">
                <strong>{t('modal.warning.title')}</strong> {t('modal.warning.description')}
              </p>
            </div>
          )}
          ...
          <div className="flex justify-end">
            <Button
              ...
              aria-label={!accepted ? t('modal.buttons.disabled') : t('modal.buttons.accept')}
            >
              {isSubmitting ? t('modal.buttons.submitting') : t('modal.buttons.accept')}
            </Button>
          </div>
