...
export function GoogleDrivePicker({
  ...
  // Handle file click
  const handleFileClick = useCallback((file: DriveFileUI) => {
    if (file.isFolder) {
  ...
  const getFileIcon = (file: DriveFileUI) => {
    ...
    if (file.mimeType.startsWith('image/')) return <ImageIcon className="w-5 h-5 text-green-500" />;
    if (file.mimeType === 'application/pdf') return <FileText className="w-5 h-5 text-red-500" />;
    ...
    <div className={cn('flex flex-col h-full', className)}>
      ...
      <div className="flex items-center justify-between gap-4 p-4 border-b">
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          <span className="truncate max-w-[150px]">{connectionStatus?.email}</span>
        </div>
