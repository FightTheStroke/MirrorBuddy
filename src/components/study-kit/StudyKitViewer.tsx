...
export function StudyKitViewer({
  ...
  const [adaptiveDifficulty, setAdaptiveDifficulty] = useState<number | null>(
    null,
  );
    ...
    <div className={cn("space-y-6", className)}>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        ...
        <TabsContent value="mindmap" className="mt-6">
          {studyKit.mindmap ? (
            <MindmapRenderer
              data={studyKit.mindmap as unknown as Record<string, unknown>}
            />
          ) : (
            <div className="bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 p-6">
              <p className="text-slate-500 text-center py-8">
      ...
      {demoCode && (
        <DemoModal
          ...
          demoCode={demoCode}
          demo={
            studyKit.demo as { title?: string; description?: string } | null
          }
        />
      )}
    </div>
