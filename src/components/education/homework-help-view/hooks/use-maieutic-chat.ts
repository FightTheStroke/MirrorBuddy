...
export function useMaieuticChat(
...
${fullPrompt}`;
    ...
    if (currentHomework) {
      const completedSteps = currentHomework.steps.filter(
        (s) => s.completed,
      ).length;
...
**IMPORTANTE**: Concentrati SOLO sul passaggio attuale. Non anticipare i successivi.`;
  ...
  const sendMaieuticMessage = useCallback(
      ...
      try {
        ...
        setMaieuticChat((prev) => [
          ...prev,
          {
            role: "assistant",
            content:
              "Scusa, c'è stato un problema. Prova a riformulare la domanda.",
            timestamp: new Date(),
          },
        ]);
      } finally {
