...
import {
  useHomeworkSessions,
  type SavedHomework as _SavedHomework,
...
export function useHomeworkHelp() {
  ...
  const [connectedMaestro, setConnectedMaestro] = useState<Maestro | null>(
    null,
  );
  ...
  const handleSubjectConfirm = useCallback(
    async (subject: Subject, maestro: Maestro | null) => {
  ...
  const handleSubmitPhoto = useCallback(
    async (photo: File): Promise<Homework> => {
      const base64 = await fileToBase64(photo);

      const response = await csrfFetch("/api/homework/analyze", {
        method: "POST",
        body: JSON.stringify({
          image: base64,
