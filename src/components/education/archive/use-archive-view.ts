...
import { type FilterType, type SortBy, updateMaterialInteraction } from './index';
...
export function useArchiveView(): UseArchiveViewReturn {
  ...
  const filtered = useMemo(() => {
    ...
    if (filter === 'bookmarked') result = result.filter((item) => item.isBookmarked);
    else if (filter !== 'all') result = result.filter((item) => item.toolType === filter);
    if (subjectFilter !== 'all') result = result.filter((item) => item.subject === subjectFilter);
    if (dateFrom) {
    ...
    if (debouncedQuery.trim()) {
      const fuse = new Fuse(result, {
        keys: [{ name: 'title', weight: 2 }, { name: 'subject', weight: 1 }, { name: 'maestroId', weight: 0.5 }, { name: 'toolType', weight: 0.5 }],
  ...
  const clearFilters = useCallback(() => { setSubjectFilter('all'); setDateFrom(''); setDateTo(''); }, []);

  const countByType = useMemo(() => {
    const counts: Record<string, number> = { all: materials.length, bookmarked: materials.filter((m) => m.isBookmarked).length };
    for (const item of materials) counts[item.toolType] = (counts[item.toolType] || 0) + 1;
    return counts;
  }, [materials]);

  return {
    filter, setFilter, sortBy, setSortBy, searchQuery, viewMode, setViewMode,
    subjectFilter, setSubjectFilter, dateFrom, setDateFrom, dateTo, setDateTo,
    isLoading, selectedItem, filtered, availableSubjects, countByType, materials,
    handleDelete, handleView, handleCloseViewer, handleNavigateToRelated,
    handleSearchChange, handleBookmark, handleRate, clearFilters,
    hasActiveFilters: subjectFilter !== 'all' || dateFrom !== '' || dateTo !== '',
  };
}
