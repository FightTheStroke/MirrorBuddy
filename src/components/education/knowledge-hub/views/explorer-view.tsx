    ...
    return (
      <div key={node.id} className="select-none">
        ...
        {hasChildren && isExpanded && (
          <div>
            {node.children!.map((child: Collection) => renderFolder(child, depth + 1))}
          </div>
        )}
      </div>
    );
        ...
        <div className="space-y-1">
          ...
          <button
            ...
            className={cn(
              'flex items-center gap-2 w-full px-2 py-1.5 text-sm rounded-md transition-colors',
              'hover:bg-slate-100 dark:hover:bg-slate-800',
              selectedCollectionId === null && 'bg-primary/10 text-primary font-medium'
            )}
            aria-label="Tutti i materiali"
          >
            <span className="w-4" />
            <Folder className="w-4 h-4 text-slate-500" />
            <span>Tutti i Materiali</span>
            <span className="ml-auto text-xs text-slate-400">
              {materials.length}
            </span>
          </button>
      ...
      <div className="flex-1 overflow-y-auto">
        <AnimatePresence mode="popLayout">
          {filteredMaterials.length === 0 ? (
            ...
            >
              Nessun materiale in questa cartella
            </motion.div>
          ) : (
