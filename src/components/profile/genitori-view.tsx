...
import { Users, RefreshCw, FileJson, FileText, AlertCircle, Loader2, Trash2 } from 'lucide-react';
...
export function GenitoriView() {
  ...
  const [activity, setActivity] = useState<ParentDashboardActivity | null>(null);
    ...
    <div className="min-h-screen bg-background">
      <div className="container mx-auto px-4 max-w-7xl">
        ...
        {(pageState === 'ready' || pageState === 'no-profile') && (
          <>
            {pageState === 'no-profile' && <WelcomeBanner highContrast={highContrast} />}

            {/* Student info + actions bar */}
            <div
              className={cn(
                'flex items-center justify-between gap-4 p-4 rounded-xl border mb-6',
                highContrast ? 'bg-black border-yellow-400' : 'bg-muted/50 border-border',
              )}
            >
              <div className="flex items-center gap-3">
                ...
                >
                  <Users
                    className={cn('w-5 h-5', highContrast ? 'text-black' : 'text-muted-foreground')}
                  />
                </div>
                <div>
                  <p
            ...
            {meta && meta.confidenceScore < 0.5 && (
              ...
              >
                <AlertCircle
                  className={cn(
                    'h-5 w-5 shrink-0',
                    highContrast ? 'text-yellow-400' : 'text-amber-600 dark:text-amber-400',
                  )}
                />
                <p
                  className={cn(
                    'text-sm',
                    highContrast ? 'text-yellow-400' : 'text-amber-700 dark:text-amber-300',
                  )}
                >
                ...
                panoramica: (() => {
                  ...
                  return (
                    <div className="space-y-6">
                      <ActivityOverview stats={data.weeklyStats} highContrast={highContrast} />
                      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        <RecentSessionsList
                          sessions={data.recentSessions}
                          highContrast={highContrast}
                        />
                        <StreakCalendar streak={data.streak} highContrast={highContrast} />
                      </div>
                    </div>
                  );
                })(),
                  ...
                  return (
                    <div className="space-y-6">
                      <SubjectsStudied
                        ...
                        highContrast={highContrast}
                      />
                      <QuizPerformance stats={data.quizStats} highContrast={highContrast} />
                      {insights && <ParentDashboard insights={insights} />}
                      <ProgressTimeline entries={diaryEntries} studentName={studentName} />
                    </div>
                  );
                })(),
                osservazioni: (
                  ...
                  <div>
                    ...
                    >
                      <p
                        className={cn(
                          'text-sm',
                          highContrast ? 'text-yellow-400' : 'text-blue-700 dark:text-blue-300',
                        )}
                      >
