...
const TOOL_BUTTONS: Array<{
  ...
  { type: "quiz", icon: HelpCircle, label: "Quiz", tooltip: "Crea quiz" },
  {
    ...
    icon: Play,
    label: "Demo",
    tooltip: "Crea simulazione interattiva",
  },
  {
    type: "flashcard",
    icon: Layers,
    label: "Flashcard",
    tooltip: "Crea flashcard",
  },
  {
    type: "summary",
    icon: FileText,
    label: "Riassunto",
    tooltip: "Scrivi un riassunto guidato",
  },
  {
    type: "search",
    icon: Search,
    label: "Cerca",
    tooltip: "Cerca su web/YouTube",
  },
...
export function ToolButtons({
  ...
  const { containerRef, hideLabels } = useContainerWidth(
    80,
    TOOL_BUTTONS.length,
  );
    ...
    >
      {TOOL_BUTTONS.map(({ type, icon: Icon, label, tooltip }) => (
        <Button
          ...
          className="h-11 min-w-[44px] flex-shrink-0 text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white"
          title={tooltip}
          aria-label={tooltip}
        >
