...
import { Headphones, Volume2, VolumeX, Play, Pause, Settings, ChevronDown } from 'lucide-react';
...
const QUICK_PRESETS: { id: AudioPreset; label: string; icon: string }[] = [
  ...
  { id: 'creative', label: 'Creativita', icon: '✨' },
  { id: 'rainy_day', label: 'Pioggia', icon: '🌧️' },
];
      ...
      {isOpen && (
        <div className="absolute right-0 top-full mt-2 w-64 bg-white dark:bg-slate-900 rounded-xl shadow-lg border border-slate-200 dark:border-slate-700 p-3 z-50">
          ...
          <button
            onClick={() => {
              setIsOpen(false);
              window.dispatchEvent(new CustomEvent('open-settings', { detail: 'ambient-audio' }));
            }}
