...
function TrialVerifyContent() {
  ...
  useEffect(() => {
    // Intentional: localStorage persists trial session ID across tabs/reloads
    // during email verification flow. Server validates independently via /api/trial/verify.
    const storedSessionId =
      typeof window !== 'undefined' ? localStorage.getItem('mirrorbuddy-trial-session-id') : null;
    if (storedSessionId) {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setStatus('error');
      setMessage(error instanceof Error ? error.message : t('trialVerify.verifyFailedLater'));
    } finally {
        ...
        <Card className="border-slate-200 dark:border-gray-700">
          ...
          <CardContent>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              >
                {isSubmitting ? t('trialVerify.submitLoading') : t('trialVerify.submit')}
              </Button>
            </form>
          </CardContent>
        </Card>
