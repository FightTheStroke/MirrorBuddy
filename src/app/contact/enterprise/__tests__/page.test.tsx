...
describe('Enterprise Contact Page', () => {
  ...
  describe('Page Rendering', () => {
    it('renders all required form fields', () => {
      ...
      expect(screen.getByPlaceholderText(/vostra azienda/i)).toBeInTheDocument();
      expect(screen.getAllByRole('combobox')).toHaveLength(2);
      expect(screen.getByText(/temi di interesse/i)).toBeInTheDocument();
      expect(screen.getByPlaceholderText(/requisiti specifici/i)).toBeInTheDocument();
    });

    it('renders sector select with required options', () => {
      render(<EnterpriseForm />);
      const sectorSelect = screen.getByLabelText(/settore/i) as HTMLSelectElement;
    ...
    it('renders employee count select with required options', () => {
      render(<EnterpriseForm />);
      const empSelect = screen.getByLabelText(/n..*dipendenti/i) as HTMLSelectElement;
  ...
  describe('Form Submission', () => {
    it('submits form with valid data', async () => {
      ...
      const sectorSelect = screen.getAllByRole('combobox')[0];
      const empSelect = screen.getAllByRole('combobox')[1];
      const messageTextarea = screen.getByPlaceholderText(/requisiti specifici/i);
