  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6">
        {GUIDES.map((guide, i) => {
          const categoryLabel = guide.category === 'coach' ? 'Coach' : guide.category === 'buddy' ? 'Buddy' : 'Maestro';
          const categoryColor = guide.category === 'coach'
          ...
          return (
            ...
            >
              {/* Category Badge */}
              <div className="mb-3">
                <span className={`inline-block px-2 py-1 rounded-full text-xs font-bold ${categoryColor}`}>
                  {categoryLabel}
                </span>
              </div>

              {/* Avatar */}
              <div className={`w-20 h-20 mx-auto mb-4 rounded-full bg-gradient-to-br ${guide.color} p-1 group-hover:scale-110 transition-transform duration-300`}>
      ...
      >
        ...
        <div className="flex flex-col md:flex-row gap-4 justify-center items-center text-sm">
          <div>
            <strong className="text-pink-600 dark:text-pink-400">5 Coaches</strong>
            <span className="text-gray-600 dark:text-gray-400"> per il metodo di studio</span>
          </div>
          <span className="hidden md:inline text-gray-400">•</span>
          <div>
            <strong className="text-orange-600 dark:text-orange-400">5 Buddies</strong>
            <span className="text-gray-600 dark:text-gray-400"> per il supporto emotivo</span>
          </div>
          <span className="hidden md:inline text-gray-400">•</span>
          <div>
            <strong className="text-blue-600 dark:text-blue-400">17 Maestri</strong>
            <span className="text-gray-600 dark:text-gray-400"> esperti di materia</span>
          </div>
        </div>
        <p className="text-xs text-gray-500 dark:text-gray-400 mt-3">
          Galileo, Darwin, Manzoni, Erodoto, Mozart, Socrate, Feynman e tanti altri ti aspettano!
        </p>
      </motion.div>
    </motion.section>
  );
