...
import { assessResponseTransparency, type TransparencyContext } from '@/lib/ai/server';
...
import { processToolCalls, buildToolChoice, filterToolDefinitions } from './tool-handler';
import {
...
function getCharacterTools(maestroId: string): string[] {
  ...
  const coach = getSupportTeacherById(maestroId as Parameters<typeof getSupportTeacherById>[0]);
...
export const POST = pipe(
  ...
  try {
    ...
    if (requestedTool) {
      const toolContext =
        requestedTool === 'demo' ? getDemoContext(maestroId) : TOOL_CONTEXT[requestedTool];
      if (toolContext) {
    ...
    const providerConfig = getActiveProvider(providerPreference, deploymentName);
    ...
    try {
      ...
      const response = NextResponse.json({
        ...
        trial: trialSessionId
          ? {
              chatsRemaining: Math.max(0, (trialCheck.chatsRemaining ?? 10) - 1),
            }
          : undefined,
      });
      response.headers.set('X-Request-ID', getRequestId(request));
      ...
      // Capture all provider errors to Sentry (were previously silently returned as 500)
      Sentry.captureException(providerError, {
        tags: { api: '/api/chat', errorType: 'provider' },
        extra: { provider: providerConfig?.provider, deploymentName, maestroId },
      });
