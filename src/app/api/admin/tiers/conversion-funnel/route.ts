...
export const GET = pipe(
  withSentry('/api/admin/tiers/conversion-funnel'),
  withAdminReadOnly,
)(async (ctx) => {
  ...
  const uniqueTrialUsers = new Set(
    tierAuditLogs
      .filter((log: TierAuditLog) => (log.changes as Record<string, string>)?.from === 'trial')
      .map((log: TierAuditLog) => log.userId),
  ).size;

  const uniqueBaseUsers = new Set(
    tierAuditLogs
      .filter((log: TierAuditLog) => (log.changes as Record<string, string>)?.to === 'base')
      .map((log: TierAuditLog) => log.userId),
  ).size;

  const uniqueProUsers = new Set(
    tierAuditLogs
      .filter((log: TierAuditLog) => (log.changes as Record<string, string>)?.to === 'pro')
      .map((log: TierAuditLog) => log.userId),
  ).size;

  // Calculate conversion rates
  const trialToBaseRate =
    uniqueTrialUsers > 0 ? (trialToBaseConversions / uniqueTrialUsers) * 100 : 0;
  const baseToProRate = uniqueBaseUsers > 0 ? (baseToProConversions / uniqueBaseUsers) * 100 : 0;
  const trialToProRate =
    uniqueTrialUsers > 0 ? (trialToProConversions / uniqueTrialUsers) * 100 : 0;
  const funnelEfficiency =
    trialToBaseConversions > 0 ? (baseToProConversions / trialToBaseConversions) * 100 : 0;
