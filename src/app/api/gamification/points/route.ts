...
export const POST = pipe(
  ...
  const validation = await validateJsonRequest(ctx.req, AwardPointsRequestSchema);
  ...
  const latestGamification = await getOrCreateGamification(userId);

  await prisma.progress.upsert({
    where: { userId },
    update: { xp: latestGamification.mirrorBucks },
    create: { userId, xp: latestGamification.mirrorBucks },
  });
