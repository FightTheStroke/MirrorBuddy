...
import { AutonomyMetrics } from './types';
import {
...
export async function GET(request: NextRequest) {
  try {
    ...
    const questionsPerSession =
      sessions.length > 0 ? Math.round(sessions.reduce((sum, s) => sum + s.questions, 0) / sessions.length) : 0;
    ...
    return NextResponse.json({ error: 'Failed to calculate autonomy metrics' }, { status: 500 });
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { userId, mindMaps, flashcards, selfAssessment, helpBehavior, methodTransfer, events, autonomyScore } =
      body;
    ...
    return NextResponse.json({ error: 'Failed to save method progress' }, { status: 500 });
  }
}

/**
 * Fetch Method Progress for store sync
 */
