...
/**
 * Zaino Sidebar (Backpack - School Metaphor)
 * Quick access to organization types and filters
 */
...
interface ZainoSidebarProps {
  ...
  onOrgTypeSelect: (type: 'materia' | 'data' | 'tipo') => void;
}
...
export function ZainoSidebar({ counts, subjects, maestros, onOrgTypeSelect }: ZainoSidebarProps) {
    ...
    <aside className="w-56 flex-shrink-0 border-r border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/50 p-4 overflow-y-auto">
      <nav aria-label="Filtri zaino">
        ...
        <div className="mt-4">
          ...
          >
            {expandedSections.organize ? <ChevronDown className="w-3 h-3" /> : <ChevronRight className="w-3 h-3" />}
            Organizza
          </button>
          {expandedSections.organize && (
            <div className="mt-1 space-y-0.5">
              <button
                onClick={() => onOrgTypeSelect('materia')}
                className="w-full flex items-center gap-2 px-3 py-1.5 rounded-lg text-left text-sm transition-colors hover:bg-blue-100 dark:hover:bg-blue-900/30 hover:text-blue-700 dark:hover:text-blue-300"
              >
                <FolderOpen className="w-4 h-4" />
                Per Materia
              </button>
              <button
                onClick={() => onOrgTypeSelect('data')}
                className="w-full flex items-center gap-2 px-3 py-1.5 rounded-lg text-left text-sm transition-colors hover:bg-green-100 dark:hover:bg-green-900/30 hover:text-green-700 dark:hover:text-green-300"
              >
                <Calendar className="w-4 h-4" />
                Per Data
              </button>
              <button
                onClick={() => onOrgTypeSelect('tipo')}
                className="w-full flex items-center gap-2 px-3 py-1.5 rounded-lg text-left text-sm transition-colors hover:bg-purple-100 dark:hover:bg-purple-900/30 hover:text-purple-700 dark:hover:text-purple-300"
              >
