 ...
 */
export async function isAzureOpenAIStressed(
  threshold: number = 80,
): Promise<boolean> {
  try {
    const limits = await getAzureOpenAILimits();
    ...
    return (
      limits.tpm.usagePercent >= threshold ||
      limits.rpm.usagePercent >= threshold
    );
  } catch (error) {
    logger.error(
      "[azure-openai-limits] Failed to check stress",
      undefined,
      error,
    );
