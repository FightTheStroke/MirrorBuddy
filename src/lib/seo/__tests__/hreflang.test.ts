...
describe("hreflang tags", () => {
  ...
  describe("buildAlternateUrls", () => {
    it("should build URL for default locale with locale prefix", () => {
      ...
      expect(urls["it"]).toBe("https://mirrorbuddy.edu/it/welcome");
    });

    it("should build URLs for all locales", () => {
      const urls = buildAlternateUrls(baseUrl, "/welcome", locales);
      expect(urls.it).toBe("https://mirrorbuddy.edu/it/welcome");
      expect(urls.en).toBe("https://mirrorbuddy.edu/en/welcome");
      expect(urls.fr).toBe("https://mirrorbuddy.edu/fr/welcome");
      expect(urls.de).toBe("https://mirrorbuddy.edu/de/welcome");
      expect(urls.es).toBe("https://mirrorbuddy.edu/es/welcome");
    });

    it("should include x-default URL", () => {
      const urls = buildAlternateUrls(baseUrl, "/welcome", locales);
      expect(urls).toHaveProperty("x-default");
      expect(urls["x-default"]).toBe("https://mirrorbuddy.edu/it/welcome");
    });

    it("should handle root path", () => {
      const urls = buildAlternateUrls(baseUrl, "/", locales);
      expect(urls.it).toBe("https://mirrorbuddy.edu/it");
      expect(urls["x-default"]).toBe("https://mirrorbuddy.edu/it");
    });

    it("should handle paths with query parameters", () => {
      const urls = buildAlternateUrls(baseUrl, "/search?q=test", locales);
      expect(urls.it).toBe("https://mirrorbuddy.edu/it/search?q=test");
      expect(urls.en).toBe("https://mirrorbuddy.edu/en/search?q=test");
    });
    ...
    it("should handle trailing slashes correctly", () => {
      ...
      expect(urls1.it).toBe("https://mirrorbuddy.edu/it/welcome");
    });
  });
  ...
  describe("generateHreflangTags", () => {
    ...
    it("should handle each page path separately", () => {
      ...
      expect(homeItTag?.href).toBe("https://mirrorbuddy.edu/it");
      expect(welcomeItTag?.href).toBe("https://mirrorbuddy.edu/it/welcome");
    });
  ...
  describe("HreflangMetadata type", () => {
    it("should validate HreflangTag structure", () => {
      const tag: HreflangTag = {
        rel: "alternate",
        hreflang: "it",
        href: "https://mirrorbuddy.edu/it/welcome",
      };
