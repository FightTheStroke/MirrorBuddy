...
describe("getRecipientPreview", () => {
  ...
  it("should handle users without profiles", async () => {
    const mockUsers = [
      { id: "user_1", email: "user1@example.com", profile: null },
    ];
...
describe("sendCampaign", () => {
  ...
  it("should check quota and block if exceeded", async () => {
    ...
    vi.mocked(prisma.emailCampaign.findUnique).mockResolvedValueOnce(
      mockCampaign as any,
    );
    vi.mocked(prisma.user.findMany).mockResolvedValueOnce(
      mockRecipients as any,
    );
  ...
  it("should skip users without consent (canSendTo returns false)", async () => {
    ...
    const { getPreferences, canSendTo } =
      await import("@/lib/email/preference-service");
