...
describe('Email Template Service', () => {
  ...
  describe('updateTemplate', () => {
    ...
    it('should validate variables when updating', async () => {
      await expect(updateTemplate('tpl-1', { variables: ['bad'] })).rejects.toThrow(
        'Unsupported template variables: bad',
      );
  ...
  describe('renderTemplate', () => {
    ...
    it('should throw error if template not found', async () => {
      mockFindUnique.mockResolvedValueOnce(null);
      await expect(renderTemplate('none', {})).rejects.toThrow('Template not found: none');
    });
  });
});
