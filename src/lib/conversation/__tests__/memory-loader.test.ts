...
import { loadPreviousContext, loadEnhancedContext, formatRelativeDate } from '../memory-loader';
...
describe('memory-loader', () => {
  ...
  describe('loadPreviousContext', () => {
    ...
    it('respects timeWindowDays filter', async () => {
      ...
      expect((whereClause?.updatedAt as Record<string, unknown>)?.gte).toBeDefined();
    });
  ...
  describe('loadEnhancedContext', () => {
    ...
    it('returns semantic memories for Pro users when query provided', async () => {
      ...
      const result = await loadEnhancedContext('user-1', 'melissa', 'pro', query);

      // Should call semantic search with correct parameters
      expect(searchRelevantSummaries).toHaveBeenCalledWith('user-1', query, 'pro', 10);
    ...
    it('skips semantic search for Base tier users', async () => {
      ...
      const result = await loadEnhancedContext('user-1', 'melissa', 'base', query);
    ...
    it('skips semantic search for Trial tier users', async () => {
      ...
      const result = await loadEnhancedContext('user-1', 'melissa', 'trial', query);
  ...
  describe('loadPreviousContext - Error Handling', () => {
    ...
    it('merges key facts with duplicate detection', async () => {
      ...
      expect(result.keyFacts).toEqual(expect.arrayContaining(['duplicate', 'unique1', 'unique2']));
    });
    ...
    it('merges topics with duplicate detection', async () => {
      ...
      expect(result.topics).toEqual(expect.arrayContaining(['duplicate', 'unique1', 'unique2']));
    });
  ...
  describe('loadEnhancedContext - Error Handling', () => {
    ...
    it('handles semantic search error and returns base memory only', async () => {
      ...
      vi.mocked(searchRelevantSummaries).mockRejectedValue(new Error('Semantic search failed'));
  ...
  describe('loadHierarchicalContext', () => {
    ...
    it('handles missing weekly summary gracefully', async () => {
      ...
      vi.mocked(prisma.hierarchicalSummary.findMany).mockResolvedValue([mockMonthlySummary]);
