...
import { createWeeklySummary, createMonthlySummary } from '../hierarchical-summarizer';
...
describe('hierarchical-summarizer', () => {
  ...
  beforeEach(() => {
    ...
    (prisma.hierarchicalSummary.create as any).mockImplementation((args: any) => {
  ...
  describe('createWeeklySummary', () => {
    ...
    it('should track topic frequency correctly', async () => {
      ...
      const mathTopic = summary.frequentTopics.find((t) => t.topic === 'Mathematics');
  ...
  describe('createMonthlySummary', () => {
    it('should aggregate 4 weeks of data into monthly summary', async () => {
      ...
      (prisma.hierarchicalSummary.findMany as any).mockResolvedValueOnce(mockWeeklySummaries);
