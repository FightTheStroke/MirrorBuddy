...
describe('TierService', () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(prisma.tierDefinition.findUnique).mockImplementation(((args: {
      ...
      if (args.where.code === 'trial') return Promise.resolve(mockTrialTier) as never;
      if (args.where.code === 'base') return Promise.resolve(mockBaseTier) as never;
      if (args.where.code === 'pro') return Promise.resolve(mockProTier) as never;
  ...
  describe('getEffectiveTier - Error Handling', () => {
    it('should fallback to Trial tier on database error when userId is null', async () => {
      vi.mocked(prisma.tierDefinition.findUnique).mockRejectedValue(new Error('Database error'));
  ...
  describe('checkFeatureAccess', () => {
    ...
    it("should return false for feature not in tier's features object", async () => {
      ...
      const result = await tierService.checkFeatureAccess('user-pro', 'nonexistent');
    ...
    it('should handle user without subscription (Base tier)', async () => {
      ...
      const result = await tierService.checkFeatureAccess('user-no-sub', 'chat');
    ...
    it('should return false on database error', async () => {
      ...
      const result = await tierService.checkFeatureAccess('user-123', 'nonexistent_feature');
    ...
    it('should cache features per tier and reuse on second call', async () => {
      ...
      const callCountAfterFirst = vi.mocked(prisma.tierDefinition.findUnique).mock.calls.length;
      ...
      const callCountAfterSecond = vi.mocked(prisma.tierDefinition.findUnique).mock.calls.length;
    ...
    it('should handle null feature value', async () => {
      ...
      const result = await tierService.checkFeatureAccess(null, 'parentDashboard');

      expect(result).toBe(false); // null is falsy
    });

    it('should handle undefined feature in features object', async () => {
      const result = await tierService.checkFeatureAccess('user-pro', 'undefinedFeature');
  ...
  describe('getAIModelForUser', () => {
    it('should return gpt-5-mini for Trial tier chat', async () => {
      const model = await tierService.getAIModelForUser(null, 'chat');

      expect(model).toBe('gpt-5-mini');
    });

    it('should return gpt-5-mini for Base tier chat', async () => {
      vi.mocked(prisma.userSubscription.findUnique).mockResolvedValue(null);
    ...
    it('should return gpt-5.2-chat for Pro tier chat', async () => {
      const mockSubscription: UserSubscription = {
        id: 'sub-123',
        userId: 'user-pro',
        tierId: 'tier-pro',
    ...
    it('should handle all model types correctly', async () => {
      ...
      // Test chat model
      const chatModel = await tierService.getAIModelForUser('user-all-models', 'chat');
      expect(chatModel).toBe('gpt-5.2-chat');
  ...
  describe('Cache Invalidation', () => {
    it('should invalidate all cached features', async () => {
      ...
      vi.mocked(prisma.userSubscription.findUnique).mockResolvedValue(updatedSubscription as never);

      // After cache invalidation, should fetch fresh data
      const result = await newTierService.checkFeatureAccess('user-cache-invalidate', 'chat');
    ...
    it('should not affect other tiers when invalidating specific tier', async () => {
      ...
      const proResult = await tierService.checkFeatureAccess('user-pro-cache', 'chat');
      const baseResult = await tierService.checkFeatureAccess('user-base-cache', 'chat');
    ...
    it('should allow caching after invalidation', async () => {
      ...
      const result = await tierService.checkFeatureAccess('user-recache', 'voice');
