...
// Namespace files expected in each locale directory
const NAMESPACES = [
  "common",
  "auth",
  "admin",
...
describe("Release Manager - i18n Validation", () => {
  ...
  it("should verify all locale directories exist", () => {
    for (const locale of locales) {
      const dirPath = path.join(messagesDir, locale);
      expect(
        fs.existsSync(dirPath),
        `Locale directory ${locale}/ should exist`,
      ).toBe(true);
    }
  });

  it("should verify all namespace files exist", () => {
    for (const locale of locales) {
      for (const ns of NAMESPACES) {
        const filePath = path.join(messagesDir, locale, `${ns}.json`);
        expect(
          fs.existsSync(filePath),
          `${locale}/${ns}.json should exist`,
        ).toBe(true);
      }
    }
  });

  it("should verify all namespace files are valid JSON", () => {
    for (const locale of locales) {
      for (const ns of NAMESPACES) {
        const filePath = path.join(messagesDir, locale, `${ns}.json`);
        const content = fs.readFileSync(filePath, "utf-8");
        expect(
          () => JSON.parse(content),
          `${locale}/${ns}.json should be valid JSON`,
        ).not.toThrow();
      }
    }
  });
  ...
  it("should verify all locales have matching keys", () => {
    ...
    for (const locale of locales) {
      if (locale !== "it") {
        ...
        const extra = Array.from(localeKeys).filter(
          (k) => !referenceKeys.has(k),
        );

        expect(missing, `${locale} should not have missing keys`).toHaveLength(
          0,
        );
...
describe("Release Manager - Maestri Verification", () => {
  ...
  it("should verify new maestri files exist", () => {
    for (const maestro of newMaestri) {
      ...
      expect(
        ...
        `${maestro}-knowledge.ts should exist`,
      ).toBe(true);
    }
  });
  ...
  it("should verify new maestri have non-empty knowledge", () => {
    for (const maestro of newMaestri) {
      ...
      expect(
        content.length,
        `${maestro}-knowledge.ts should have content`,
      ).toBeGreaterThan(100);
      expect(
        content,
        `${maestro}-knowledge.ts should export knowledge`,
      ).toMatch(/export\s+const\s+\w+_KNOWLEDGE/);
    }
  });
...
describe("Release Manager - Locale Loading Test", () => {
  ...
  it("should load all locales without errors", async () => {
    for (const locale of LOCALES) {
      ...
      expect(
        messages,
        `Locale ${locale} should load successfully`,
      ).toBeDefined();
      expect(typeof messages, `Locale ${locale} should be an object`).toBe(
        "object",
      );
      expect(
        Object.keys(messages).length,
        `Locale ${locale} should have keys`,
      ).toBeGreaterThan(0);
    }
  });
  ...
  it("should have consistent structure across all locales", () => {
    ...
    for (const locale of LOCALES) {
      if (locale !== "it") {
        expect(
          structures[locale],
          `${locale} structure should match reference`,
        ).toEqual(referenceStructure);
      }
    }
  });
});
