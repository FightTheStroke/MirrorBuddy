...
import { mergeCanonicalMetadata, createLocalizedMetadata } from '../metadata-helpers';
...
describe('canonical-urls integration', () => {
  ...
  describe('locale separation', () => {
    ...
    it('each locale gets unique canonical URL', () => {
      const canonicals = locales.map((locale) => generateCanonicalUrl(locale, testPath));
  ...
  describe('SEO compliance', () => {
    ...
    it('query parameters are preserved in canonical URLs', () => {
      const canonicalWithParams = generateCanonicalUrl('it', '/search?q=test&page=1');
