...
describe('Invite Service', () => {
  beforeEach(() => {
    ...
    vi.mocked(prisma.$transaction).mockImplementation((fn: unknown) =>
      (fn as Function)({
        ...
        user: { create: vi.fn().mockResolvedValue({ id: 'user-123' }) },
      }),
    );
  });
  ...
  describe('getPendingInvites', () => {
    it('should return pending invites ordered by creation date', async () => {
      ...
      vi.mocked(prisma.inviteRequest.findMany).mockResolvedValue(mockInvites as never);
  ...
  describe('notifyAdminNewRequest', () => {
    it('should send email notification for valid request', async () => {
      ...
      vi.mocked(prisma.inviteRequest.findUnique).mockResolvedValue(mockRequest as never);
  ...
  describe('bulkRejectInvites', () => {
    it('should process multiple reject requests with reason', async () => {
      ...
      const result = await bulkRejectInvites(['req-1', 'req-2'], 'admin-123', 'Not eligible');
