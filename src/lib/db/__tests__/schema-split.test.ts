...
const EXPECTED_MODELS = [
  // user.prisma
  'User', 'Profile', 'Settings', 'GoogleAccount', 'AccessibilitySettings',
  'OnboardingState', 'PomodoroStats',
  // education.prisma
  'FlashcardProgress', 'QuizResult', 'Learning', 'StudySession', 'MethodProgress',
  // conversations.prisma
  'Conversation', 'Message', 'ToolOutput',
  // content.prisma
  'Material', 'Collection', 'Tag', 'MaterialTag', 'CreatedTool', 'HtmlSnippet', 'StudyKit',
  // gamification.prisma
  'Progress', 'UserGamification', 'Achievement', 'UserAchievement', 'DailyStreak', 'PointsTransaction',
  // scheduling.prisma
  'Notification', 'PushSubscription', 'StudySchedule', 'ScheduledSession',
  'CustomReminder', 'CalendarEvent',
  // learning-path.prisma
  'LearningPath', 'LearningPathTopic', 'TopicStep', 'TopicAttempt',
  // analytics.prisma
  'TelemetryEvent', 'RateLimitEvent', 'SafetyEvent',
  // insights.prisma
  'StudentInsightProfile', 'ProfileAccessLog', 'ParentNote', 'HomeworkSession',
  // rag.prisma
  'ContentEmbedding', 'MaterialEdge', 'Concept', 'MaterialConcept',
];
...
describe('Prisma Schema Split', () => {
  ...
  describe('Schema Content', () => {
    ...
    it('should have all expected models across schema files', () => {
      const allContent = EXPECTED_SCHEMA_FILES
  ...
  describe('Domain Boundaries', () => {
    ...
    it('should have Achievement model in gamification.prisma', () => {
      const content = readFileSync(join(SCHEMA_DIR, 'gamification.prisma'), 'utf-8');
      expect(content).toContain('model Achievement {');
    });

    it('should have LearningPath model in learning-path.prisma', () => {
      const content = readFileSync(join(SCHEMA_DIR, 'learning-path.prisma'), 'utf-8');
