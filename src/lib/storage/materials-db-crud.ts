 ...
 * Create, read, update, delete operations for stored materials
 */
...
export async function saveMaterial(
  ...
  if (metadata.format === 'image') {
    const { generateThumbnail } = await import('./materials-db-utils');
 ...
 */
export async function getMaterialThumbnail(
  id: string
): Promise<Blob | undefined> {
  const db = await getMaterialsDB();
  const file = await db.get('files', id);
