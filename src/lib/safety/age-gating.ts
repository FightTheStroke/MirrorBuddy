 ...
 */
const TOPIC_MATRIX: Record<ContentTopic, Record<AgeBracket, TopicSensitivity>> = {
  basic_education: {
    elementary: 'safe',
    middle: 'safe',
    highschool: 'safe',
 ...
 */
const ADAPTATION_GUIDANCE: Record<TopicSensitivity, string> = {
 ...
 */
const ALTERNATIVE_SUGGESTIONS: Partial<Record<ContentTopic, string>> = {
...
export function filterForAge(text: string, age: number): AgeGateResult {
  ...
  const mostRestrictive = results.reduce((worst, current) => {
    const severityOrder: TopicSensitivity[] = ['safe', 'moderate', 'restricted', 'blocked'];
