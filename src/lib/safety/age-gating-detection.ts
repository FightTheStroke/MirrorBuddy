...
import type { ContentTopic, AgeGateResult } from './age-gating-types';
import { checkAgeGate } from './age-gating-core';
...
export function filterForAge(text: string, age: number): AgeGateResult {
  ...
  const mostRestrictive = results.reduce((worst, current) => {
    const severityOrder: ('safe' | 'moderate' | 'restricted' | 'blocked')[] = [
      'safe',
      'moderate',
      'restricted',
      'blocked',
