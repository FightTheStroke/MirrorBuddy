...
export const DE_PATTERNS: PIIPatternCategory = {
  ...
  address: [
    ...
    /\b[\p{L}\s-]+(?:straße|strasse)\b(?:\s+\d+[a-z]?)?\b/giu,
    // eslint-disable-next-line security/detect-unsafe-regex -- Intentional PII detection pattern for German addresses
    /\b[\p{L}\s-]+str\.\b(?:\s+\d+[a-z]?)?\b/giu,
    // eslint-disable-next-line security/detect-unsafe-regex -- Intentional PII detection pattern for German addresses
    /\b[\p{L}\s-]+platz\b(?:\s+\d+[a-z]?)?\b/giu,
    // eslint-disable-next-line security/detect-unsafe-regex -- Intentional PII detection pattern for German addresses
    /\b[\p{L}\s-]+weg\b(?:\s+\d+[a-z]?)?\b/giu,
    // eslint-disable-next-line security/detect-unsafe-regex -- Intentional PII detection pattern for German addresses
    /\b[\p{L}\s-]+allee\b(?:\s+\d+[a-z]?)?\b/giu,
    // eslint-disable-next-line security/detect-unsafe-regex -- Intentional PII detection pattern for German addresses
    /\b[\p{L}\s-]+gasse\b(?:\s+\d+[a-z]?)?\b/giu,
  ],
