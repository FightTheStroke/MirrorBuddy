...
Rispondi SOLO con JSON valido in questo formato:
{
  ...
  if (!jsonMatch) {
    logger.error('Failed to parse topic analysis JSON', { response: result.content.substring(0, 500) });
  ...
  try {
    ...
    if (analysisData.topics.length < 2 || analysisData.topics.length > 5) {
      logger.warn('Topic count outside expected range', { count: analysisData.topics.length });
    }
    ...
    const topics: IdentifiedTopic[] = analysisData.topics.map(
      (t: {
        ...
        keyConcepts: Array.isArray(t.keyConcepts) ? t.keyConcepts.map(String) : [],
        estimatedDifficulty: validateDifficulty(t.estimatedDifficulty),
        order: typeof t.order === 'number' ? t.order : index + 1,
        textExcerpt: t.textExcerpt ? String(t.textExcerpt).substring(0, 500) : '',
      })
    );
    ...
    const result: TopicAnalysisResult = {
      ...
      suggestedOrder: Array.isArray(analysisData.suggestedOrder)
        ...
        : topics.map((t) => t.id),
      totalEstimatedMinutes: typeof analysisData.totalEstimatedMinutes === 'number'
 ...
 */
function validateDifficulty(value: unknown): 'basic' | 'intermediate' | 'advanced' {
  const normalized = String(value).toLowerCase();
  if (normalized === 'basic' || normalized === 'base' || normalized === 'facile') {
    return 'basic';
  }
  if (normalized === 'intermediate' || normalized === 'intermedio' || normalized === 'medio') {
    return 'intermediate';
  }
  if (normalized === 'advanced' || normalized === 'avanzato' || normalized === 'difficile') {
    return 'advanced';
  }
 ...
 */
export function orderTopicsPedagogically(topics: IdentifiedTopic[]): IdentifiedTopic[] {
