...
import { startTopic, completeTopic, getPathProgress, resetTopic } from '../progress-manager';
...
describe('progress-manager', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    // Make $transaction call the callback with the same prisma mock
  ...
  describe('completeTopic', () => {
    ...
    it('should throw for locked topic [F-14]', async () => {
      ...
      await expect(completeTopic('topic-1')).rejects.toThrow('Cannot complete a locked topic');
    });
  });
  ...
  describe('getPathProgress', () => {
    ...
    it('should throw for non-existent path [F-14]', async () => {
      ...
      await expect(getPathProgress('nonexistent')).rejects.toThrow('Path not found');
    });
  });
