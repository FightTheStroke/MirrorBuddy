...
describe("material-linker", () => {
  ...
  describe("findRelatedMaterials", () => {
    ...
    it("should find related materials by keyword match [F-08]", async () => {
      const mockMaterials = [
        {
          ...
          topic: "Roma Antica",
          searchableText:
            "Il senato romano era il consiglio principale della Repubblica",
      ...
      expect(result[0].relatedMaterials[0].title).toBe(
  ...
  describe("formatRelatedMaterialsMessage", () => {
    ...
    it("should format single material message", () => {
      ...
      expect(result).toBe(
        'Hai già studiato questo argomento in "Storia Romana"',
      );
    });
    ...
    it("should format multiple materials message", () => {
      ...
      expect(result).toBe(
        'Questo si collega a quello che hai studiato in: "Doc A", "Doc B"',
      );
    });
