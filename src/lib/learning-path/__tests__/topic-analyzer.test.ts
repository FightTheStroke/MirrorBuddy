...
vi.mock("@/lib/tier/tier-service", () => ({
  tierService: {
    getFeatureAIConfigForUser: vi.fn(() =>
      Promise.resolve({ model: "gpt-4o", temperature: 0.5, maxTokens: 2500 }),
    ),
  },
}));
...
describe("topic-analyzer", () => {
  ...
  describe("analyzeTopics", () => {
    ...
    it("should suggest pedagogical order [F-09]", async () => {
      ...
      const basicTopic = result.topics.find(
        (t) => t.estimatedDifficulty === "basic",
      );
      const advancedTopic = result.topics.find(
        (t) => t.estimatedDifficulty === "advanced",
      );
    ...
    it("should reject empty text", async () => {
      await expect(analyzeTopics("", "title")).rejects.toThrow(
