...
describe('Orchestrator Helpers', () => {
  ...
  describe('withTimeout', () => {
    ...
    it('should clear timeout even on rejection', async () => {
      ...
      await expect(withTimeout(promise, 1000, 'test_tool')).rejects.toThrow('failure');
  ...
  describe('extractErrorMessage', () => {
    ...
    it('should stringify object error', () => {
      ...
      expect(extractErrorMessage(error)).toBe('{"code":"ERR_001","message":"Object error"}');
    });
  ...
  describe('createErrorToolResult', () => {
    it('should create error result with Error object', () => {
      ...
      const result = createErrorToolResult('test_tool', new Error('Test failure'));

      expect(result.success).toBe(false);
      expect(result.error).toContain('test_tool');
      expect(result.error).toContain('Test failure');
      expect(result.output).toBeUndefined();
      expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('test_tool'));
