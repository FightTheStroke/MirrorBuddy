...
describe("tool-output-storage", () => {
  ...
  describe("saveToolOutput", () => {
    ...
    it("should include toolId when provided", async () => {
      ...
      const result = await saveToolOutput(
        conversationId,
        "quiz",
        mockData,
        toolId,
    ...
    it("should throw on database error", async () => {
      vi.mocked(prisma.toolOutput.create).mockRejectedValue(
        new Error("DB error"),
      );

      await expect(
        saveToolOutput(conversationId, "mindmap", {}),
  ...
  describe("getToolOutputs", () => {
    ...
    it("should throw on database error", async () => {
      vi.mocked(prisma.toolOutput.findMany).mockRejectedValue(
        new Error("DB error"),
      );
  ...
  describe("getToolOutputsByType", () => {
    ...
    it("should throw on database error", async () => {
      ...
      await expect(
        getToolOutputsByType(conversationId, "quiz"),
      ).rejects.toThrow("DB error");
    });
  });
  ...
  describe("getToolOutputCount", () => {
    ...
    it("should return 0 on error", async () => {
      vi.mocked(prisma.toolOutput.count).mockRejectedValue(
        new Error("DB error"),
      );
  ...
  describe("getToolOutputStats", () => {
    it("should return stats grouped by tool type", async () => {
      ...
      vi.mocked(prisma.toolOutput.findMany).mockResolvedValue(
        mockOutputs as any,
      );
  ...
  describe("deleteToolOutputs", () => {
    ...
    it("should throw on database error", async () => {
      vi.mocked(prisma.toolOutput.deleteMany).mockRejectedValue(
        new Error("DB error"),
      );

      await expect(deleteToolOutputs(conversationId)).rejects.toThrow(
        "DB error",
      );
    });
  });
