...
describe("Mindmap Export", () => {
  ...
  describe("exportMindmap", () => {
    ...
    describe("Nested structures", () => {
      it("should handle deeply nested children", async () => {
        ...
        expect(
          jsonContent.root.children[0].children[0].children[0].children[0].text,
        ).toBe("Level 4");

        const mdResult = await exportMindmap(deepMindmap, {
          format: "markdown",
        });
  ...
  describe("Edge cases", () => {
    ...
    it("should preserve node colors", async () => {
      ...
      const jsonResult = await exportMindmap(coloredMindmap, {
        format: "json",
      });
      const jsonContent = JSON.parse(await jsonResult.blob.text());
      ...
      const fmResult = await exportMindmap(coloredMindmap, {
        format: "freemind",
      });
