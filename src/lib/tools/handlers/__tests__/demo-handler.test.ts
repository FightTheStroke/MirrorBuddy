...
describe("validateCode", () => {
  ...
  describe("Dangerous pattern detection", () => {
    ...
    it("should detect Function constructor", () => {
      ...
      expect(validateCode(dangerousCode1).violations).toContain(
        "Function constructor",
      );

      expect(validateCode(dangerousCode2).safe).toBe(false);
      expect(validateCode(dangerousCode2).violations).toContain(
        "Function constructor",
      );
    });
...
describe("sanitizeHtml", () => {
  describe("Script tag removal", () => {
    it("should remove script tags with content", async () => {
    ...
    it("should remove script tags case-insensitively", async () => {
    ...
    it("should remove script tags with mixed case", async () => {
      const html = '<ScRiPt>alert("xss")</sCrIpT>';
    ...
    it("should handle script tags without closing tags", async () => {
    ...
    it("should handle nested script-like content", async () => {
      const html =
        '<div><script>var x = "<script>nested</script>"</script></div>';
  ...
  describe("Event handler removal", () => {
    it("should remove onclick handler", async () => {
    ...
    it("should remove onload handler", async () => {
    ...
    it("should remove onerror handler", async () => {
    ...
    it("should remove multiple event handlers", async () => {
    ...
    it("should handle event handlers with various spacing", async () => {
      const html1 = '<div onclick="x()">Test</div>';
      const html2 = '<div onclick = "x()">Test</div>';
      const html3 = '<div onclick  =  "x()">Test</div>';

      expect(await sanitizeHtml(html1)).not.toContain("onclick");
      expect(await sanitizeHtml(html2)).not.toContain("onclick");
      expect(await sanitizeHtml(html3)).not.toContain("onclick");
    });

    it("should be case-insensitive for event handlers", async () => {
      const html = '<button ONCLICK="alert(1)">Click</button>';
  ...
  describe("Dangerous URL scheme removal", () => {
    ...
    it("should remove javascript: protocol case-insensitively", async () => {
      const html = '<a href="JavaScript:alert(1)">Link</a>';
    ...
    it("should remove vbscript: protocol", async () => {
    ...
    it("should remove data: protocol in href", async () => {
      const html =
    ...
    it("should handle HTML entity encoded protocols", async () => {
      const html1 =
        '<a href="&#106;&#97;&#118;&#97;&#115;&#99;&#114;&#105;&#112;&#116;&#58;alert(1)">Link</a>';
      const html2 =
        '<a href="&#x6A;&#x61;&#x76;&#x61;&#x73;&#x63;&#x72;&#x69;&#x70;&#x74;&#x3A;alert(1)">Link</a>';

      const sanitized1 = await sanitizeHtml(html1);
      const sanitized2 = await sanitizeHtml(html2);
    ...
    it("should preserve safe protocols", async () => {
  ...
  describe("Edge cases", () => {
    ...
    it("should handle HTML with multiple issues", async () => {
      const html = `
        <div onclick="x()">
...
describe("decodeHtmlEntities", () => {
  ...
  it("should decode decimal HTML entities", async () => {
    const html = '<a href="&#106;avascript:alert(1)">Link</a>';
    const sanitized = await sanitizeHtml(html);
