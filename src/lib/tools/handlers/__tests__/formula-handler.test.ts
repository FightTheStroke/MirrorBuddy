...
const { capturedHandler, mockChatCompletion, mockRegisterToolHandler } = vi.hoisted(() => {
...
import { isLatex, validateLatex, generateLatexFromDescription } from '../formula-handler';
...
describe('formula-handler', () => {
  ...
  describe('generateLatexFromDescription', () => {
    ...
    it('generates LaTeX from description', async () => {
      mockChatCompletion.mockResolvedValue({
        content: '{"latex": "a^{2} + b^{2} = c^{2}", "explanation": "Pythagorean theorem"}',
      });
