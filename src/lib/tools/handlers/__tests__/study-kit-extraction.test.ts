...
describe('Study Kit Extraction', () => {
  ...
  describe('extractTextFromPDF', () => {
    ...
    it('should throw error for empty buffer', async () => {
      await expect(extractTextFromPDF(Buffer.from(''))).rejects.toThrow('Empty or invalid PDF buffer');
    });
    ...
    it('should handle getText failure', async () => {
      ...
      await expect(extractTextFromPDF(Buffer.from('PDF'))).rejects.toThrow('Failed to parse PDF: Text extraction failed');
    });
    ...
    it('should handle getInfo failure', async () => {
      ...
      await expect(extractTextFromPDF(Buffer.from('PDF'))).rejects.toThrow('Failed to parse PDF: Info extraction failed');
    });
    ...
    it('should convert non-Error exceptions to string', async () => {
      ...
      await expect(extractTextFromPDF(Buffer.from('PDF'))).rejects.toThrow('Failed to parse PDF: String error');
    });

    it('should handle null buffer', async () => {
      // @ts-expect-error Testing invalid input
      await expect(extractTextFromPDF(null)).rejects.toThrow('Empty or invalid PDF buffer');
    });
  });
});
