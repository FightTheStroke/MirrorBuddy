...
function getArchiveHandler(): ((args: Record<string, unknown>, context?: any) => Promise<ToolExecutionResult>) | null {
  return (globalThis as any).__archiveHandler ?? null;
}

function requireArchiveHandler(): (args: Record<string, unknown>, context?: any) => Promise<ToolExecutionResult> {
...
describe('Archive Handler', () => {
  ...
  describe('Empty Search Results', () => {
    it('should handle no results for query', async () => {
      ...
      expect(data.message).toContain('Non ho trovato materiali con "nonexistent"');
    });
