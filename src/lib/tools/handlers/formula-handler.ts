...
import type { FormulaData, ToolExecutionResult } from '@/types/tools';
...
function validateLatex(latex: string): { valid: boolean; error?: string } {
  ...
  for (const char of latex) {
    ...
    if (char === '}') braceCount--;
    if (braceCount < 0) {
      return { valid: false, error: 'Unbalanced braces: closing } without opening {' };
    }
  }
  if (braceCount !== 0) {
  ...
  if (leftCount !== rightCount) {
    return { valid: false, error: `Unbalanced delimiters: ${leftCount} \\left vs ${rightCount} \\right` };
  }
 ...
 */
registerToolHandler('create_formula', async (args): Promise<ToolExecutionResult> => {
  ...
  if (inputLatex) {
    ...
    displayMode = latex.length > 30 || latex.includes('\\int') || latex.includes('\\sum')
      ? 'block'
      : 'inline';
  ...
  // Case 2: Description provided, generate LaTeX
    ...
    if (isLatex(inputDescription)) {
      ...
      logger.info('Formula handler: Description was LaTeX', { latexLength: latex.length });
    } else {
      ...
      if (!generated || !generated.latex) {
        return {
          ...
          toolId: nanoid(),
          toolType: 'formula',
          error: 'Non sono riuscito a generare la formula. Prova a specificare il LaTeX direttamente.',
        };
      }
