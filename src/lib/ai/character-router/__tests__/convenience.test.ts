...
vi.mock('../selection', () => ({
  ...
  getBuddyForStudent: vi.fn(() => ({
    id: 'buddy-1',
    name: 'Max',
    getGreeting: (profile: ExtendedStudentProfile) => `Ehi ${profile.name}!`,
...
describe('character router convenience', () => {
    ...
    it('should return buddy greeting using profile', () => {
      ...
      const result = mockRoutingResult({
        character: {
          id: 'buddy-1',
          name: 'Max',
          getGreeting: (p: ExtendedStudentProfile) => `Ehi ${p.name}! Tutto bene?`,
          getSystemPrompt: () => '',
        },
        characterType: 'buddy',
        confidence: 0.9,
  ...
  describe('suggestCharacterSwitch', () => {
    it('should suggest maestro switch', () => {
      ...
      const result = suggestCharacterSwitch('buddy', 'maestro', profile, 'Hai bisogno di aiuto con la matematica.');

      expect(result.character).toBeDefined();
      expect(result.message).toContain('Hai bisogno di aiuto con la matematica.');
    ...
    it('should suggest coach switch', () => {
      ...
      const result = suggestCharacterSwitch('maestro', 'coach', profile, 'Sembra che tu abbia bisogno di supporto.');
    ...
    it('should suggest buddy switch', () => {
      ...
      const result = suggestCharacterSwitch('maestro', 'buddy', profile, 'Vuoi fare una pausa?');
    ...
    it('should default to coach for unknown type', () => {
      ...
      const result = suggestCharacterSwitch('maestro', 'unknown' as 'coach', profile, 'Test reason');
