    ...
    describe('with Azure provider', () => {
      ...
      it('should include tools in request when provided', async () => {
        global.fetch = vi.fn().mockResolvedValue({
          ...
          json: async () => ({
            choices: [{
              message: { content: '', tool_calls: [{ id: 'call_1', type: 'function', function: { name: 'test', arguments: '{}' } }] },
      ...
      it('should handle API errors', async () => {
        ...
        await expect(chatCompletion(mockMessages, mockSystemPrompt))
          .rejects.toThrow('Azure OpenAI error (429): Rate limit exceeded');
      });
    ...
    describe('with Ollama provider', () => {
      ...
      it('should throw when model is not available', async () => {
        ...
        global.fetch = vi.fn()
          ...
          .mockResolvedValueOnce({ // isOllamaModelAvailable
            ok: true,
            json: async () => ({ models: [] }),
          });

        await expect(chatCompletion(mockMessages, mockSystemPrompt))
          .rejects.toThrow('Ollama model "llama3.2" not found');
      });
      ...
      it('should call Ollama API and return response', async () => {
        global.fetch = vi.fn()
          ...
          .mockResolvedValueOnce({ // actual chat completion
            ok: true,
            json: async () => ({
              choices: [{
      ...
      it('should handle Ollama API errors', async () => {
        global.fetch = vi.fn()
          ...
          .mockResolvedValueOnce({ // actual chat completion fails
            ok: false,
            text: async () => 'Internal server error',
          });

        await expect(chatCompletion(mockMessages, mockSystemPrompt))
          .rejects.toThrow('Ollama error: Internal server error');
      });
    });
  });
