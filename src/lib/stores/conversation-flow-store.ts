...
import { getDefaultSupportTeacher } from '@/data/support-teachers';
  ...
  persist(
    (set, get) => ({
      ...
      switchToBuddy: (profile) => {
        const { getBuddyForStudent } = require('@/lib/ai/character-router');
      ...
      goBack: (profile) => {
        ...
        // Import the character getters
        const { getMaestroById } = require('@/data/maestri-full');
        const { getSupportTeacherById } = require('@/data/support-teachers');
        const { getBuddyById } = require('@/data/buddy-profiles');
