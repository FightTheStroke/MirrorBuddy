...
export const useCalendarStore = create<CalendarState>()((set, get) => ({
  ...
  getUpcomingEvents: (days) => {
    ...
    return get()
      .events.filter(
        (e) =>
          !e.completed && new Date(e.date) >= now && new Date(e.date) <= future,
      )
  ...
  getSuggestedMaestri: () => {
    ...
    const suggestions: Map<
      string,
      { count: number; nearestDays: number; types: Set<string> }
    > = new Map();
    ...
    return Array.from(suggestions.entries())
      .map(([maestroId, data]) => {
        ...
        if (data.nearestDays <= 1) {
          priority = "high";
          reason =
            data.types.has("exam") || data.types.has("test")
