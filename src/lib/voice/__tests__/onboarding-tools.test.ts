...
describe('onboarding-tools', () => {
  ...
  describe('executeOnboardingTool', () => {
    it('sets student name successfully', async () => {
      const result = await executeOnboardingTool('set_student_name', { name: 'Marco' });
    ...
    it('rejects empty name', async () => {
      const result = await executeOnboardingTool('set_student_name', { name: '' });
    ...
    it('rejects name too short', async () => {
      const result = await executeOnboardingTool('set_student_name', { name: 'A' });
    ...
    it('rejects name too long', async () => {
      ...
      const result = await executeOnboardingTool('set_student_name', { name: longName });
    ...
    it('sets student age successfully', async () => {
      const result = await executeOnboardingTool('set_student_age', { age: 12 });
    ...
    it('rejects age above 19', async () => {
      const result = await executeOnboardingTool('set_student_age', { age: 20 });
    ...
    it('sets school level successfully', async () => {
      const result = await executeOnboardingTool('set_school_level', { level: 'media' });

      expect(result.success).toBe(true);
      expect(mockStoreState.updateData).toHaveBeenCalledWith({ schoolLevel: 'media' });
    });

    it('rejects invalid school level', async () => {
      const result = await executeOnboardingTool('set_school_level', { level: 'universita' });
