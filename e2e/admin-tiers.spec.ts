...
test.describe('Admin Tiers - Create New Tier', () => {
  test('should display tier creation form with all required fields', async ({ adminPage }) => {
    await adminPage.goto('/admin/tiers/new');
    await adminPage.waitForLoadState('domcontentloaded');

    await expect(
      adminPage.locator('h1, h2').filter({ hasText: /Nuovo Piano|Crea Piano/i }),
    ).toBeVisible();
    ...
    await expect(adminPage.locator('input[name="monthlyPriceEur"]')).toBeVisible();
    await expect(adminPage.locator('input[name="sortOrder"]')).toBeVisible();
    await expect(adminPage.locator('input[name="isActive"], input[type="checkbox"]')).toBeVisible();

    await expect(adminPage.locator('button').filter({ hasText: /Salva|Crea/i })).toBeVisible();
    await expect(
      adminPage.locator('button, a').filter({ hasText: /Annulla|Indietro/i }),
    ).toBeVisible();
  });

  test('should navigate back to tiers list on cancel', async ({ adminPage }) => {
    await adminPage.goto('/admin/tiers/new');
    await adminPage.waitForLoadState('domcontentloaded');
  ...
  test('should create new tier with API mock', async ({ adminPage }) => {
    ...
    await adminPage.locator('textarea[name="description"]').fill('Test description');
    await adminPage.locator('input[name="monthlyPriceEur"]').fill('9.99');
    await adminPage.locator('input[name="sortOrder"]').fill('1');

    const submitButton = adminPage.locator('button').filter({ hasText: /Salva|Crea/i });
...
test.describe('Admin Tiers - Edit Existing Tier', () => {
  test('should display tier edit form with pre-filled values', async ({ adminPage }) => {
    await adminPage.route('**/api/admin/tiers/*', async (route) => {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
    ...
    await expect(adminPage.locator('h1, h2').filter({ hasText: /Modifica Piano/i })).toBeVisible();
    ...
    await expect(adminPage.locator('input[name="name"]')).toHaveValue('Premium');
  ...
  test('should update tier with API mock', async ({ adminPage }) => {
    ...
    const submitButton = adminPage.locator('button').filter({ hasText: /Salva|Aggiorna/i });
    await submitButton.click();
  ...
  test('should navigate to edit page from tiers table', async ({ adminPage }) => {
...
test.describe('Admin Tiers - Features Section', () => {
  test('should display features section with checkboxes', async ({ adminPage }) => {
    await adminPage.route('**/api/admin/tiers/tier-123', async (route) => {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
    ...
    await expect(adminPage.locator('h2').filter({ hasText: /Funzionalità/i })).toBeVisible();

    const videoVisionCheckbox = adminPage.locator('input[name="feature-video_vision"]');
    await expect(videoVisionCheckbox).toBeVisible();
    await expect(videoVisionCheckbox).toBeChecked();

    const voiceEnabledCheckbox = adminPage.locator('input[name="feature-voice_enabled"]');
    await expect(voiceEnabledCheckbox).toBeVisible();
    await expect(voiceEnabledCheckbox).toBeChecked();

    const pdfExportCheckbox = adminPage.locator('input[name="feature-pdf_export"]');
    await expect(pdfExportCheckbox).toBeVisible();
    await expect(pdfExportCheckbox).not.toBeChecked();

    await expect(adminPage.locator('input[name="feature-flashcards"]')).toBeVisible();
    await expect(adminPage.locator('input[name="feature-mindmaps"]')).toBeVisible();
    await expect(adminPage.locator('input[name="feature-quizzes"]')).toBeVisible();
    await expect(adminPage.locator('input[name="feature-homework_help"]')).toBeVisible();
    await expect(adminPage.locator('input[name="feature-formula_tool"]')).toBeVisible();
    await expect(adminPage.locator('input[name="feature-chart_tool"]')).toBeVisible();
  });
  ...
  test('should display feature labels and descriptions', async ({ adminPage }) => {
    await adminPage.route('**/api/admin/tiers/tier-123', async (route) => {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
    ...
    await expect(adminPage.locator('text=Enable webcam video analysis')).toBeVisible();

    await expect(adminPage.locator('text=Voice Chat')).toBeVisible();
    await expect(adminPage.locator('text=Enable voice conversations with maestri')).toBeVisible();
