...
test.describe('Google Drive API', () => {
  test('GET /api/auth/google/status returns connection status', async ({ request }) => {
    const response = await request.get(`/api/auth/google/status?userId=${TEST_USER_ID}`);
  ...
  test('GET /api/google-drive/files returns file list when connected', async ({ request }) => {
    // First check if connected
    const statusResponse = await request.get(`/api/auth/google/status?userId=${TEST_USER_ID}`);
    ...
    const response = await request.get(`/api/google-drive/files?userId=${TEST_USER_ID}`);
  ...
  test('GET /api/google-drive/files with folderId=shared returns shared files', async ({ request }) => {
    const statusResponse = await request.get(`/api/auth/google/status?userId=${TEST_USER_ID}`);
    const status = await statusResponse.json();
    ...
    const response = await request.get(`/api/google-drive/files?userId=${TEST_USER_ID}&folderId=shared`);
  ...
  test('GET /api/google-drive/files with search query', async ({ request }) => {
    ...
    const response = await request.get(`/api/google-drive/files?userId=${TEST_USER_ID}&search=test`);
  ...
  test('GET /api/google-drive/files/[id]/download requires userId', async ({ request }) => {
    const response = await request.get('/api/google-drive/files/test-file-id/download');
...
test.describe('Google Drive UI - Study Kit', () => {
  test('Study Kit shows Google Drive option when connected', async ({ page }) => {
    const errors: string[] = [];
    page.on('console', (msg) => {
      if (msg.type() === 'error') {
        const text = msg.text();
    ...
    const driveTab = page.locator('button, [role="tab"]').filter({ hasText: /Google Drive/i }).first();
    ...
    if (await driveTab.isVisible()) {
      ...
      if (await studyKitCard.isVisible()) {
        ...
        const driveOption = page.locator('text=/Da Google Drive|Google Drive|Connetti/i').first();
  ...
  test('Google Drive picker shows files when opened', async ({ page }) => {
    ...
    if (await driveTab.isVisible()) {
      ...
      const fileList = page.locator('[class*="file"], [class*="folder"], text=/Il mio Drive|Condivisi con me|Connetti/i');
      await expect(fileList.first()).toBeVisible({ timeout: 5000 });
    }
  });

  test('Google Drive picker shows "Il mio Drive" and "Condivisi con me" tabs', async ({ page }) => {
    await page.goto('/');
    await page.waitForLoadState('networkidle');
    ...
    if (await driveTab.isVisible()) {
      ...
      const myDriveTab = page.locator('button').filter({ hasText: /Il mio Drive/i });
      const sharedTab = page.locator('button').filter({ hasText: /Condivisi con me/i });
...
test.describe('Google Drive UI - Homework Help', () => {
  test('Homework Help shows Google Drive upload option', async ({ page }) => {
    ...
    const homeworkBtn = page.locator('button, a').filter({ hasText: /Compiti|Homework/i }).first();
    ...
    if (await homeworkBtn.isVisible()) {
      ...
      const uploadSection = page.locator('text=/Carica|Upload|Google Drive|PDF/i').first();
