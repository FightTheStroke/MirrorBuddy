...
test.describe('Login/Logout Authentication Flow', () => {
  ...
  test('Login with valid credentials redirects to home', async ({ page, request: _request }) => {
    ...
    expect(['http://localhost:3000/', 'http://localhost:3000/home'].some(url => page.url().startsWith(url))).toBeTruthy();
  });
  ...
  test('Protected page redirects unauthenticated users to login', async ({ context, page }) => {
    // Clear auth cookies
    await context.clearCookies();
