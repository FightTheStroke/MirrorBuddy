...
export async function verifyPageLocale(
  ...
  if (htmlLang !== expectedLocale) {
    errors.push(
      `HTML lang="${htmlLang}" does not match expected "${expectedLocale}"`,
    );
  }
  ...
  if (!urlContainsLocale) {
    errors.push(
      `URL "${url}" does not contain locale prefix "/${expectedLocale}/"`,
    );
  }
  ...
  const cookieValue = localeCookie?.value || null;

  // Cookie is optional - only error if it exists but is wrong
  if (cookieValue && cookieValue !== expectedLocale) {
    errors.push(
      `NEXT_LOCALE cookie="${cookieValue}" does not match expected "${expectedLocale}"`,
    );
  }
 ...
 */
export async function testLocaleDetection(
  page: Page,
  expectedLocale: Locale,
): Promise<boolean> {
