...
test.describe('Welcome Page Language Switcher - F-69', () => {
  ...
  test('should show all 5 languages in dropdown', async ({ page }) => {
    ...
    await expect(page.getByRole('menuitem', { name: /italiano/i })).toBeVisible();
    await expect(page.getByRole('menuitem', { name: /english/i })).toBeVisible();
    await expect(page.getByRole('menuitem', { name: /français/i })).toBeVisible();
    await expect(page.getByRole('menuitem', { name: /deutsch/i })).toBeVisible();
  ...
  test('should set NEXT_LOCALE cookie and redirect on selection', async ({ page }) => {
    await page.goto('/it/welcome');
  ...
  test.skip('should persist language across page reloads', async ({ page }) => {
    ...
    const currentOption = page.locator('[role="menuitem"][aria-current="true"]');
  ...
  test.describe('Browser Language Detection', () => {
    test('should respect browser Accept-Language header', async ({ browser }) => {
