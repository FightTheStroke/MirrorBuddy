...
const TEST_USER_PREFIXES = ['e2e-test-user-', 'admin-test-session-', 'test-user-'];
...
async function globalTeardown() {
  ...
  if (!connectionString) {
    throw new Error('TEST_DATABASE_URL is required for E2E cleanup to avoid production writes.');
  }
  ...
  try {
    ...
    const [convResult, msgResult, userResult] = await prisma.$transaction([
      prisma.conversation.deleteMany({ where: { isTestData: true } }),
      prisma.message.deleteMany({ where: { isTestData: true } }),
      prisma.user.deleteMany({ where: { isTestData: true } }),
    ]);

    totalDeleted += userResult.count;

    if (userResult.count > 0 || convResult.count > 0) {
      console.log(
        `  Cleaned ${userResult.count} test users, ${convResult.count} conversations, ${msgResult.count} messages (by isTestData flag)`,
      );
    }
