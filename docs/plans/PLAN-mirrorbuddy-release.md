...
| D-70 | Contrasto colore insufficiente su card home "Fare i compiti" (1.37-1.55:1 invece di 4.5:1, classi `dark:` che leakano in light mode)                                                                                           | triage E2E post-deploy (`home-intent-a11y.spec.ts`, `home-intent-dsa-a11y.spec.ts`)                                                                                                                                                  | —                                  | 🔴 aperto — bug WCAG 2.1 AA reale in `home-intent-chooser.tsx` (ultimo commit `31cc7e29`, non toccato in questa sessione), non una regressione. Serve fix mirato alle classi `dark:text-slate-300`/`dark:bg-slate-800` in light mode                                                                                                                                                                                                                                                                                                                                                                          |
| D-71 | hreflang mancante sulla pagina `/terms` (le altre pagine testate nello stesso spec passano)                                                                                                                                    | triage E2E post-deploy (`hreflang.spec.ts:94`)                                                                                                                                                                                       | —                                  | 🔴 aperto — bug localizzato pre-esistente, non regressione globale                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| D-72 | `prisma.config.ts` ignora `DEV_DATABASE_URL`/override shell, legge sempre `DATABASE_URL` da `.env` (Supabase condiviso) anche con un override locale esportato                                                                 | audit /focus-group post-fix (near-miss: `prisma db push --accept-data-loss` ha quasi toccato il DB Supabase condiviso, rollback automatico su conflitto, verificato via `prisma migrate diff` read-only che nulla è stato applicato) | —                                  | 🔴 aperto — rischio DX/safety alto: chiunque lavori in un worktree con DB locale può colpire accidentalmente il DB condiviso. Serve far rispettare la precedenza dell'env locale in `prisma.config.ts`                                                                                                                                                                                                                                                                                                                                                                                                        |
