...
| C2     | Cleanup codice: rimuovere provider Claude morto da router.ts (P2-4)      | Igiene/doc        | Basso (era "Alto", sovrastimato — vedi nota) | S      | Sonnet                                               |
...
| C12    | Tier cleanup DEC-06 (D-35)                                               | qualità           | Basso                                        | S      | Sonnet                                               |
...
- **D-53 — funnel non misurato (TJ.9):** definire i 5 eventi chiave (landing_view, trial_start, first_message, first_tool, return_visit), verificare che il tracking `lib/funnel` spari davvero (oggi mockato negli E2E), creare dashboard minima (Grafana in stack) per i target ≤3/≤2 interazioni; eventi in `docs/analytics/FUNNEL.md`. Vive accanto a D-54 (stesse serie Grafana). _(Sonnet.)_
