...
`SYSTEM_MAESTRO_KB`. Note that no committed script actually writes them there — see G-6, a
finding of this audit. This SOP governs the content of those files regardless of whether
they are served from pgvector or embedded in the prompt.
...
| Artefact                                              | Role                                                                                                                     |
| ----------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| `apps/web/src/data/maestri/*-knowledge.ts` (32 files) | Authored didactic content; the intended RAG corpus (see G-6 — the seeding pipeline does not currently populate pgvector) |
...
| `scripts/seed-maestri-knowledge-vectors.ts`           | Chunks and embeds the didactic bulk — but does not persist it (G-6)                                                      |
...
| G-6 | The seeding pipeline for the corpus this SOP governs is **broken in two independent ways**, so nothing in `*-knowledge.ts` currently reaches pgvector. (a) `extract-mini-kb.ts` resolves `../src/data/maestri`, a pre-monorepo path that no longer exists. (b) `seed-maestri-knowledge-vectors.ts` chunks, embeds and **counts** — it never calls `storeEmbedding` or Prisma, so it persists nothing even when run. Consequence: `maestro-knowledge-retriever.ts` reads `SYSTEM_MAESTRO_KB` from pgvector and, per ADR 0033, **degrades silently to an empty string** when it finds nothing — so an unpopulated index looks exactly like a working one. Diagnosed during this audit, corrected after review (the first version of this finding named only the stale path). Not fixed here: it needs its own card and a live database to verify end-to-end. | Repoint the extractor, add the missing persistence step, then verify a seed run against a real database | Roberdan | 30 Sep 2026 |

G-6 is an operational break, not an IP one — but it means the provenance controls in this SOP currently govern content that is not being served from the vector store; the rest are **documentation and review gaps, not known infringements**: the corpus is
