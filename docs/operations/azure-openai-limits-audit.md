...
### 1. gpt4o-mini-deployment (Primary Chat Model)
...
| **Capacity** | 1K TPM |
| **Status** | Succeeded |
| **Created** | 2025-12-18T20:31:24Z |

**Rate Limits**:
- **Requests Per Minute (RPM)**: 6 (1 request every 10 seconds)
- **Tokens Per Minute (TPM)**: 1,000
- **Renewal Period**: 10 seconds (requests), 60 seconds (tokens)
...
**Usage in MirrorBuddy**: Currently not used (mini model in `.env`).

**Note**: Very low capacity (1K TPM, 6 RPM) - suitable only for low-volume premium features.

---

### 3. gpt-5-mini (Experimental)
...
**Usage in MirrorBuddy**: Not configured in `.env`, experimental deployment.

**Note**: Extremely low capacity (1 RPM) - likely for testing only.
...
- gpt4o-mini-deployment: 100K TPM
- gpt-4o: 1K TPM
- gpt-5-mini: 1K TPM
- **Total**: 102K TPM
...
**Current**: gpt4o-mini-deployment (100K TPM capacity)
...
AZURE_OPENAI_REALTIME_DEPLOYMENT_MINI=gpt-4o-mini-realtime  # 80-90% cheaper
```
