...
describe('POST /api/webhooks/resend', () => {
  ...
  it('handles email.delivered: updates status and deliveredAt', async () => {
    ...
    mockVerify.mockReturnValue(payload);
    vi.mocked(prisma.emailRecipient.findFirst).mockResolvedValue(mockRecipient('r-del', 'msg_del'));
    vi.mocked(prisma.emailRecipient.update).mockResolvedValue({} as any);
    vi.mocked(prisma.emailEvent.create).mockResolvedValue({} as any);
  ...
  it('handles email.bounced: updates status and bouncedAt', async () => {
    ...
    mockVerify.mockReturnValue(payload);
    vi.mocked(prisma.emailRecipient.findFirst).mockResolvedValue(mockRecipient('r-bnc', 'msg_bnc'));
  ...
  it('handles duplicate events idempotently', async () => {
    ...
    mockVerify.mockReturnValue(payload);
    vi.mocked(prisma.emailRecipient.findFirst).mockResolvedValue(mockRecipient('r-dup', 'msg_dup'));
    vi.mocked(prisma.emailRecipient.update).mockResolvedValue({} as any);
    vi.mocked(prisma.emailEvent.create).mockResolvedValue({} as any);
  ...
  it('returns 200 even when database update fails', async () => {
    ...
    mockVerify.mockReturnValue(payload);
    vi.mocked(prisma.emailRecipient.findFirst).mockResolvedValue(mockRecipient('r-err', 'msg_err'));
    vi.mocked(prisma.emailRecipient.update).mockRejectedValue(new Error('Database error'));
