...
export const GET = pipe(withSentry('/api/auth/sso/microsoft/callback'))(async (ctx) => {
  ...
  try {
    const callbackResult = await provider.callback(
      { code, state, codeVerifier: session.codeVerifier, nonce: session.nonce },
      {
    ...
    const ssoResult = await handleSSOCallback(callbackResult.userInfo, 'microsoft');
