...
vi.mock('@/lib/trial/trial-request', async (original) => ({
  ...
  findOwnedTrialSession: vi.fn().mockResolvedValue({ id: 'session-123' }),
}));
...
vi.mock('next/headers', () => ({
  ...
  cookies: vi.fn(async () => ({
    get: vi.fn((name: string) => {
      if (name === TRIAL_CONSENT_COOKIE) return { value: '{"accepted":true}' };
...
describe('PATCH /api/trial/email', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(findOwnedTrialSession).mockImplementation(async () => null);
  });

  it('saves email to trial session', async () => {
    vi.mocked(findOwnedTrialSession).mockImplementation(
      async () =>
        ({
          id: 'session-123',
        }) as Awaited<ReturnType<typeof findOwnedTrialSession>>,
    );
    const mockSession = {
      id: 'session-123',
      email: null,
    ...
    expect(updateTrialEmail).toHaveBeenCalledWith('session-123', 'user@example.com');
  ...
  it('returns 404 if session not found', async () => {
    vi.mocked(updateTrialEmail).mockRejectedValue(new Error('Session not found'));
  ...
  it('allows updating email even if already set', async () => {
    ...
    expect(updateTrialEmail).toHaveBeenCalledWith('session-123', 'new@example.com');
    expect(requestTrialEmailVerification).toHaveBeenCalledWith('session-123');
  });
});
