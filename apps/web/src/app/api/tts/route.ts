 ...
 */
async function generateAzureAudio15TTS(text: string, voice: TTSVoice): Promise<ArrayBuffer> {
  const endpoint = process.env.AZURE_OPENAI_ENDPOINT!;
  const apiKey = process.env.AZURE_OPENAI_API_KEY!;
  const apiVersion = process.env.AZURE_OPENAI_API_VERSION || '2024-08-01-preview';
 ...
 *
 * Tries the best configured provider first and, on failure, cascades to the
...
export const POST = pipe(
  ...
  if (!provider) {
    return NextResponse.json(
      {
        error: 'TTS not configured. Set OPENAI_API_KEY or AZURE_OPENAI_TTS_DEPLOYMENT',
        fallback: true,
      },
      { status: 503 },
    );
