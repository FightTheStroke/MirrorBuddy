...
export default function ResetPasswordPage() {
  ...
  const [formState, setFormState] = useState<FormState>(token ? 'idle' : 'error');
  const [errorMessage, setErrorMessage] = useState(token ? '' : t('resetPassword.invalidToken'));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // A reset revokes old sessions and issues nothing, including in this browser.
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-green-50 to-blue-50 dark:from-slate-900 dark:to-slate-800 p-4">
        <div className="max-w-md w-full bg-white dark:bg-slate-800 rounded-2xl shadow-xl p-8 text-center space-y-6">
          ...
          <p className="text-slate-600 dark:text-slate-300">{t('resetPassword.successMessage')}</p>
          <p className="text-sm text-slate-500 dark:text-slate-400">{t('redirectingToLogin')}</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-md w-full bg-white dark:bg-slate-800 rounded-2xl shadow-xl p-8 space-y-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <Button type="submit" disabled={!canSubmit} className="w-full">
            {formState === 'submitting' ? 'Resetting...' : t('resetPassword.submitButton')}
          </Button>
        </form>
      </div>
    </div>
