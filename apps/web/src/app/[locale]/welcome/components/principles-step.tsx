...
const PRINCIPLE_KEYS = ['learnYourWay', 'maestriCount', 'neverAlone', 'funLearning'] as const;

const PRINCIPLE_ICONS: Record<string, LucideIcon> = {
  learnYourWay: Lightbulb,
  maestriCount: Users,
  neverAlone: Heart,
  funLearning: Sparkles,
};

const PRINCIPLE_COLORS: Record<string, string> = {
  learnYourWay: 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-600 dark:text-yellow-400',
  maestriCount: 'bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400',
  neverAlone: 'bg-pink-100 dark:bg-pink-900/30 text-pink-600 dark:text-pink-400',
  funLearning: 'bg-purple-100 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400',
};
      ...
      <CardContent className="p-8 space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-gray-500 dark:text-gray-400 text-sm">{t('whatMakesUsSpecial')}</p>
          </div>
          {/* Voice toggle */}
          <button
            onClick={toggleMute}
          ...
          >
            {isVoiceMuted ? (
              ...
              <Volume2
                className={`w-5 h-5 text-pink-600 dark:text-pink-400 ${isPlaying ? 'animate-pulse' : ''}`}
              />
            )}
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {PRINCIPLE_KEYS.map((key, index) => {
            ...
            const color = PRINCIPLE_COLORS[key];
            return (
              <motion.div
                key={key}
        ...
        >
          <p className="text-gray-700 dark:text-gray-300 italic">
            &ldquo;{t('melissaQuote', { name: data.name })}&rdquo;
          </p>
