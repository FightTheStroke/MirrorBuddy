...
function TierCard({
  ...
  return (
    ...
    >
      ...
      <div className="mb-6">
        <h3 className="text-2xl font-bold text-gray-900">{t(`tiers.${tierKey}.name`)}</h3>
        <p className="mt-2 text-sm text-gray-600">{t(`tiers.${tierKey}.description`)}</p>
      </div>

      <div className="mb-6">
        <span className="text-4xl font-bold text-gray-900">{price}</span>
        {isPro && <span className="text-gray-600">{t(`tiers.${tierKey}.period`)}</span>}
      </div>

      <ul className="mb-8 space-y-3">
        {featuresFor(t, tierKey).map((feature, idx) => {
          return (
