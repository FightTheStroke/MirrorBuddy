...
export default async function RootLayout({
  ...
  try {
    ...
    Sentry.captureException(err, {
      tags: { component: 'RootLayout', operation: 'getLocale' },
    });
  ...
  // Wrapped in try/catch to prevent cold-start module loading failures from crashing the page
  let messages: Awaited<ReturnType<typeof getMessages>> = {};
  try {
    messages = await getMessages();
  } catch (err) {
    Sentry.captureException(err, {
      tags: { component: 'RootLayout', operation: 'getMessages' },
      extra: { locale },
    });
  }
