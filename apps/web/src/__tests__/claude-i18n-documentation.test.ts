 ...
 * The canonical agent instructions live in AGENTS.md, which references
 * `.claude/rules/i18n.md` (auto-loaded). CLAUDE.md is a thin pointer to it.
...
describe('AGENTS.md i18n Documentation', () => {
  test('should reference i18n modular rule in AGENTS.md', () => {
    expect(agentsContent).toMatch(/i18n/i);
    expect(agentsContent).toMatch(/\.claude\/rules/);
  });

  test('should include supported locales (it, en, fr, de, es)', () => {
    expect(agentsContent).toMatch(/it.*en.*fr.*de.*es|locales.*it.*en.*fr.*de.*es/i);
  });
  ...
  test('should include useTranslations hook usage patterns in i18n rule', () => {
    expect(i18nRuleContent).toMatch(/useTranslations|hook.*usage|getTranslations/i);
  });
  ...
  test('should include formality rules reference', () => {
    expect(agentsContent).toMatch(/ADR|formality|adr-0064|formal|informal/i);
  });
