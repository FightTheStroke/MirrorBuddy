...
import { encryptTokenWithKey, encryptPIIWithKey } from '../key-rotation-helpers';

describe('Key Rotation Service', () => {
  const oldTokenKey = 'old-token-key-must-be-at-least-32-chars-long-for-security';
  const newTokenKey = 'new-token-key-must-be-at-least-32-chars-long-for-security';
  ...
  describe('rotateTokenEncryptionKey', () => {
    it('should rotate GoogleIntegration tokens successfully', async () => {
      const mockAccessToken = await encryptTokenWithKey('access123', oldTokenKey);
      const mockRefreshToken = await encryptTokenWithKey('refresh456', oldTokenKey);

      vi.mocked(prisma.googleAccount.count).mockResolvedValue(2);
      ...
      expect(prisma.googleAccount.update).toHaveBeenCalledTimes(2);
    });
    ...
    it('should handle empty database', async () => {
      ...
      expect(result.phase).toBe('complete');
      expect(prisma.googleAccount.findMany).not.toHaveBeenCalled();
    });
