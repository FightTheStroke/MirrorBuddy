...
// Mock the secret reader BEFORE importing pii-encryption to avoid caching issues
...
import { encryptPII, decryptPII, hashPII, _resetPIIKeyCache } from '../pii-encryption';
...
describe('PII Encryption', () => {
  ...
  beforeEach(() => {
    ...
    process.env.PII_ENCRYPTION_KEY = 'test-key-for-pii-encryption-at-least-32-chars-long';
  });
  ...
  describe('encryptPII and decryptPII', () => {
    ...
    it('should throw meaningful error for invalid ciphertext', async () => {
      ...
      await expect(decryptPII(invalidCiphertext)).rejects.toThrow(/PII decryption failed/i);
    });
    ...
    it('should throw error for corrupted ciphertext', async () => {
      ...
      await expect(decryptPII(corrupted)).rejects.toThrow(/PII decryption failed/i);
    });
  ...
  describe('Environment key fallback', () => {
    it('should use ENCRYPTION_KEY if PII_ENCRYPTION_KEY not set', async () => {
      delete process.env.PII_ENCRYPTION_KEY;
      process.env.ENCRYPTION_KEY = 'fallback-encryption-key-at-least-32-chars-long';
  ...
  describe('Version Prefix', () => {
    ...
    it('should handle future version prefixes gracefully', async () => {
      ...
      await expect(decryptPII(futureVersion)).rejects.toThrow(/PII decryption failed/i);
    });
  });
  ...
  describe('Cryptographic Properties', () => {
    it('should use authenticated encryption (verify integrity)', async () => {
      ...
      await expect(decryptPII(tampered)).rejects.toThrow(/PII decryption failed/i);
    });
