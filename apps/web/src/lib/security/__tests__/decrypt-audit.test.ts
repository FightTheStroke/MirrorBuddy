...
import { logDecryptAccess, logBulkDecryptAccess, type DecryptAuditContext } from '../decrypt-audit';
...
describe('Decrypt Audit Logging', () => {
  ...
  describe('logDecryptAccess', () => {
    ...
    it('should not throw on Prisma error', async () => {
      ...
      vi.mocked(prisma.complianceAuditEntry.create).mockRejectedValue(
      ...
      expect(prisma.complianceAuditEntry.create).toHaveBeenCalled();
    });
    ...
    it('should handle multiple concurrent log calls', async () => {
      ...
      await vi.waitFor(
        () => {
          expect(prisma.complianceAuditEntry.create).toHaveBeenCalledTimes(3);
        },
        { timeout: 1000 },
      );
    });
