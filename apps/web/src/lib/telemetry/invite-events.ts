...
import { hasAnalyticsConsent, onAnalyticsRevoked } from './optional-analytics-client';
import { logger } from '@/lib/logger';

interface InviteEvent {
  type: 'invite_request_submitted' | 'invite_approved' | 'invite_rejected' | 'invite_first_login';
  timestamp: string;
 ...
 */
export function trackInviteRequestSubmitted(email: string, hasTrialSession: boolean): void {
 ...
 */
export function trackInviteRejected(requestId: string, adminId: string, hasReason: boolean): void {
  if (!hasAnalyticsConsent()) return;
  const event: InviteEvent = {
 ...
 */
export function trackInviteFirstLogin(userId: string, migratedTrialData: boolean): void {
