...
function getTriggerDetails(trigger: EscalationTrigger): {
  ...
  const details: Record<EscalationTrigger, { prefix: string; description: string }> = {
    crisis_detected: {
    ...
    age_gate_bypass: {
      ...
      description: 'Pattern detected suggesting potential age verification bypass attempt.',
    },
    session_termination: {
      prefix: '[HIGH] Session Forcibly Terminated',
      description: 'Session was terminated due to safety threshold. Review logs.',
    },
  };
 ...
 */
function buildEmailHtml(event: EscalationEvent, trigger: EscalationTrigger): string {
 ...
 */
function buildEmailText(event: EscalationEvent, trigger: EscalationTrigger): string {
  const { prefix, description } = getTriggerDetails(trigger);
  const timestamp = event.timestamp.toISOString();
 ...
 */
export async function notifyAdmin(event: EscalationEvent, adminEmail?: string): Promise<boolean> {
  // An explicit address still wins: some escalations are routed to a named
  // guardian rather than to the whole administrator group.
  const to = adminEmail ? [adminEmail] : await getAdminRecipients();
