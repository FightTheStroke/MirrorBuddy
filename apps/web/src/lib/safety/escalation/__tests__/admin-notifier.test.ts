...
// Recipients now come from the administrators in the database; an explicit
// address still wins, so most tests here exercise the explicit path.
...
describe('admin-notifier', () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    getAdminRecipients.mockResolvedValue(['configured-admin@test.com']);
    // Reset env
  ...
  describe('notifyAdmin', () => {
    ...
    it('should send email for severe_content_filter trigger', async () => {
      ...
      expect(sendEmail).toHaveBeenCalledWith(
        expect.objectContaining({
          subject: 'MirrorBuddy Escalation: [HIGH] Severe Content Filter Violation',
        }),
      );
    });
    ...
    it('should send email for age_gate_bypass trigger', async () => {
      ...
      expect(sendEmail).toHaveBeenCalledWith(
        expect.objectContaining({
          subject: 'MirrorBuddy Escalation: [HIGH] Potential Age Verification Bypass',
        }),
      );
    });
    ...
    it('reaches every administrator when no address is given', async () => {
      const { sendEmail } = await import('@/lib/email');
      vi.mocked(sendEmail).mockResolvedValue({
        success: true,
        messageId: 'msg-6',
      });
      getAdminRecipients.mockResolvedValue(['first-admin@test.com', 'second-admin@test.com']);
      ...
      expect(sendEmail).toHaveBeenCalledWith(
        expect.objectContaining({
          to: ['first-admin@test.com', 'second-admin@test.com'],
        }),
      );
    });
