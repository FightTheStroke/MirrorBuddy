...
describe('Trial Email Service', () => {
  ...
  describe('updateTrialEmail', () => {
    it('updates email for existing session', async () => {
      ...
      vi.mocked(prisma.trialSession.findUnique).mockResolvedValue(mockSession as any);
      ...
      vi.mocked(prisma.trialSession.update).mockResolvedValue(updatedSession as any);
    ...
    it('throws error if session not found', async () => {
      ...
      await expect(updateTrialEmail('nonexistent', 'user@example.com')).rejects.toThrow(
        'Session not found',
      );
    });
