...
describe('onboarding-tools', () => {
  ...
  describe('executeOnboardingTool', () => {
    ...
    it('rejects age below 8', async () => {
      const result = await executeOnboardingTool('set_student_age', { age: 7 });

      expect(result.success).toBe(false);
      expect(result.error).toContain('8 e 18');
    });

    it('rejects age above 18', async () => {
      const result = await executeOnboardingTool('set_student_age', {
        age: 19,
      });
