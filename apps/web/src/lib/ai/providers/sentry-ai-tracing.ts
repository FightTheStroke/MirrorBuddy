 ...
 * @param model - Model identifier (e.g., 'gpt-5-mini', 'llama3.2')
  ...
  return Sentry.startSpan(
    {
      ...
      attributes: {
        ...
        'gen_ai.request.messages': JSON.stringify(truncateMessages(allMessages)),
        // Configuration
    ...
    async (span) => {
      try {
        ...
        if (result.content) {
          ...
          const truncatedResponse =
            result.content.length > 500
              ...
              : result.content;
          span.setAttribute('gen_ai.response.text', JSON.stringify([truncatedResponse]));
        }
        ...
        if (result.finish_reason) {
          span.setAttribute('gen_ai.response.finish_reason', result.finish_reason);
        }
        ...
        if (result.tool_calls && result.tool_calls.length > 0) {
          span.setAttribute('gen_ai.response.tool_calls', result.tool_calls.length);
          span.setAttribute(
