...
describe("hreflang tags", () => {
  ...
  describe("buildAlternateUrls", () => {
    it("should build URL for default locale with locale prefix", () => {
      ...
      expect(urls["it"]).toBe("https://test.example.com/it/welcome");
    });

    it("should build URLs for all locales", () => {
      const urls = buildAlternateUrls(baseUrl, "/welcome", locales);
      expect(urls.it).toBe("https://test.example.com/it/welcome");
      expect(urls.en).toBe("https://test.example.com/en/welcome");
      expect(urls.fr).toBe("https://test.example.com/fr/welcome");
      expect(urls.de).toBe("https://test.example.com/de/welcome");
      expect(urls.es).toBe("https://test.example.com/es/welcome");
    });

    it("should include x-default URL", () => {
      const urls = buildAlternateUrls(baseUrl, "/welcome", locales);
      expect(urls).toHaveProperty("x-default");
      expect(urls["x-default"]).toBe("https://test.example.com/it/welcome");
    });

    it("should handle root path", () => {
      const urls = buildAlternateUrls(baseUrl, "/", locales);
      expect(urls.it).toBe("https://test.example.com/it");
      expect(urls["x-default"]).toBe("https://test.example.com/it");
    });

    it("should handle paths with query parameters", () => {
      const urls = buildAlternateUrls(baseUrl, "/search?q=test", locales);
      expect(urls.it).toBe("https://test.example.com/it/search?q=test");
      expect(urls.en).toBe("https://test.example.com/en/search?q=test");
    });
    ...
    it("should handle trailing slashes correctly", () => {
      ...
      expect(urls1.it).toBe("https://test.example.com/it/welcome");
    });
  });
  ...
  describe("generateHreflangTags", () => {
    ...
    it("should include self-referencing hreflang", () => {
      ...
      expect(itTag?.href).toBe("https://test.example.com/it/welcome");
    });
    ...
    it("should include x-default tag", () => {
      ...
      expect(xDefault?.href).toBe("https://test.example.com/it/welcome");
    });
    ...
    it("should handle each page path separately", () => {
      ...
      expect(homeItTag?.href).toBe("https://test.example.com/it");
      expect(welcomeItTag?.href).toBe("https://test.example.com/it/welcome");
    });
  ...
  describe("HreflangMetadata type", () => {
    it("should validate HreflangTag structure", () => {
      const tag: HreflangTag = {
        rel: "alternate",
        hreflang: "it",
        href: "https://test.example.com/it/welcome",
      };
