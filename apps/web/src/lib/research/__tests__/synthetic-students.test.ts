...
describe('synthetic-students', () => {
  describe('SYNTHETIC_PROFILES', () => {
    it('should have exactly 5 profiles', () => {
      expect(SYNTHETIC_PROFILES).toHaveLength(5);
    });
    ...
    it('should have unique DSA profiles', () => {
      ...
      expect(new Set(dsas).size).toBe(5);
    });
    ...
    it.each(SYNTHETIC_PROFILES)('$name should have valid fields', (profile) => {
      ...
      expect(profile.responsePatterns.frustrationThreshold).toBeLessThanOrEqual(1);
      expect(profile.responsePatterns.typicalBehaviors.length).toBeGreaterThan(0);
      expect(profile.responsePatterns.frustrationCues.length).toBeGreaterThan(0);
      expect(profile.responsePatterns.engagementSignals.length).toBeGreaterThan(0);
    });
  });

  describe('Elena cerebral_palsy profile', () => {
    it('should exist with correct dsaProfile', () => {
      const elena = SYNTHETIC_PROFILES.find((p) => p.name === 'Elena-CerebralPalsy-11');
      expect(elena).toBeDefined();
      expect(elena?.dsaProfile).toBe('cerebral_palsy');
    });

    it('should have correct age and schoolYear', () => {
      const elena = SYNTHETIC_PROFILES.find((p) => p.name === 'Elena-CerebralPalsy-11');
      expect(elena?.age).toBe(11);
      expect(elena?.schoolYear).toBe(6);
    });

    it('should have auditory learning style', () => {
      const elena = SYNTHETIC_PROFILES.find((p) => p.name === 'Elena-CerebralPalsy-11');
      expect(elena?.learningStyle).toBe('auditory');
    });
  });
  ...
  describe('getProfileByDsa', () => {
    ...
    it('should find cerebral_palsy profile', () => {
      const result = getProfileByDsa('cerebral_palsy');
      expect(result).toBeDefined();
      expect(result?.name).toBe('Elena-CerebralPalsy-11');
