...
vi.mock('@/lib/feature-flags/policy-writer', () => ({
  beginFeaturePolicyWrite: vi.fn(() => ({
    completion: Promise.resolve({
      persistence: 'confirmed',
      superseded: false,
      scope: 'instance',
      effective: { killSwitch: false, status: 'enabled', enabledPercentage: 100 },
    }),
  })),
}));
...
describe('degradation-service', () => {
  ...
  describe('recoverFeature', () => {
    it('removes feature from degraded list after acknowledgement', async () => {
      degradeFeature('rag_enabled', 'cache', 'test');
      expect(getFallbackBehavior('rag_enabled')).toBe('cache');

      await recoverFeature('rag_enabled', 'recovered');
      expect(getFallbackBehavior('rag_enabled')).toBeNull();
    });

    it('updates degradation level on confirmed recovery', async () => {
      degradeFeature('rag_enabled', 'cache', 'test');
      degradeFeature('pdf_export', 'simplified', 'test');
      expect(getDegradationState().level).toBe('partial');

      await recoverFeature('rag_enabled', 'recovered');
      await recoverFeature('pdf_export', 'recovered');
      expect(getDegradationState().level).toBe('none');
    });
  });
