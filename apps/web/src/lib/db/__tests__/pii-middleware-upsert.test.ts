...
describe('PII Encryption on Upsert', () => {
  ...
  it('encrypts User.email in upsert create data', async () => {
    const mockQuery = vi.fn((args) => Promise.resolve({ id: 'user1', ...args.create }));
  ...
  it('encrypts User.email in upsert update data', async () => {
    const mockQuery = vi.fn((args) => Promise.resolve({ id: 'user1', ...args.update }));
    ...
    expect(piiEncryption.encryptPII).toHaveBeenCalledWith('test@example.com');
    expect(piiEncryption.encryptPII).toHaveBeenCalledWith('updated@example.com');
    expect(mockQuery).toHaveBeenCalledWith(
      expect.objectContaining({
  ...
  it('decrypts result after upsert', async () => {
    ...
    expect(piiEncryption.decryptPII).toHaveBeenCalledWith('pii:v1:encrypted_test@example.com', {
      throwOnError: false,
    });
  ...
  it('skips encryption for non-PII models on upsert', async () => {
    const mockQuery = vi.fn((args) => Promise.resolve({ id: '1', ...args.create }));
  ...
  it('encrypts Profile.name in upsert', async () => {
    const mockQuery = vi.fn((args) => Promise.resolve({ id: 'p1', ...args.create }));
