...
function loadLocaleMessages(locale: string): Record<string, unknown> {
  const localeDir = path.join(process.cwd(), 'apps', 'web', 'messages', locale);
...
describe('Release Manager - i18n Validation', () => {
  const messagesDir = path.join(process.cwd(), 'apps', 'web', 'messages');
  const locales = ['it', 'en', 'fr', 'de', 'es'];

  it('should verify all locale directories exist', () => {
    for (const locale of locales) {
      const dirPath = path.join(messagesDir, locale);
      expect(fs.existsSync(dirPath), `Locale directory ${locale}/ should exist`).toBe(true);
    }
  });
  ...
  it('should verify all namespace files exist', () => {
    for (const locale of locales) {
      for (const ns of NAMESPACES) {
        ...
        expect(fs.existsSync(filePath), `${locale}/${ns}.json should exist`).toBe(true);
      }
    }
  });
...
describe('Release Manager - Maestri Verification', () => {
  ...
  it('should verify new maestri have non-empty knowledge', () => {
    for (const maestro of newMaestri) {
      ...
      expect(content, `${maestro}-knowledge.ts should export knowledge`).toMatch(
        /export\s+const\s+\w+_KNOWLEDGE/,
      );
    }
  });
...
describe('Release Manager - Locale Loading Test', () => {
  ...
  it('should have consistent structure across all locales', () => {
    ...
    for (const locale of LOCALES) {
      if (locale !== 'it') {
        expect(structures[locale], `${locale} structure should match reference`).toEqual(
          referenceStructure,
        );
      }
    }
...
function extractKeysFromObject(obj: Record<string, unknown>, prefix = ''): Set<string> {
  ...
  Object.entries(obj).forEach(([key, value]) => {
    ...
    if (typeof value === 'object' && value !== null && !Array.isArray(value)) {
      extractKeysFromObject(value as Record<string, unknown>, fullKey).forEach((k) => keys.add(k));
    } else {
      keys.add(fullKey);
    }
  });
