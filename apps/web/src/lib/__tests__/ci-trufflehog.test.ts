...
describe('CI Workflow - TruffleHog Secret Scanning (F-09)', () => {
  ...
  describe('TruffleHog Configuration', () => {
    it('secret-scanning should checkout code with full history', () => {
      const job = ciWorkflow.jobs['secret-scanning'];
      const checkoutStep = job.steps.find((s: any) => s.uses?.startsWith('actions/checkout'));
    ...
    it('TruffleHog should scan changed commits on push and PR events', () => {
      // Uses conditional expressions to pick the right SHAs:
      // - For push: github.event.before / github.event.after
      // - For pull_request: PR base/head SHAs
      const job = ciWorkflow.jobs['secret-scanning'];
      const truffleStep = job.steps.find((s: any) =>
        s.uses?.includes('trufflesecurity/trufflehog'),
      );
      expect(truffleStep.with.base).toContain('github.event.before');
      expect(truffleStep.with.base).toContain('pull_request');
      expect(truffleStep.with.head).toContain('github.event.after');
      expect(truffleStep.with.head).toContain('pull_request');
    });
  });
