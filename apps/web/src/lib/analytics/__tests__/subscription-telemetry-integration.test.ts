...
import { trackSubscriptionEvent, subscriptionTelemetry } from '../subscription-telemetry';
...
describe('Subscription Telemetry Integration', () => {
  ...
  describe('Event tracking workflow', () => {
    it('tracks full lifecycle: created -> upgraded -> cancelled', () => {
      ...
      const created = subscriptionTelemetry.helpers.createCreated('user-123', 'tier-free');
      ...
      vi.clearAllMocks();
      mockFetch.mockResolvedValue({ ok: true });
      const cancelled = subscriptionTelemetry.helpers.createCancelled('user-123', 'tier-pro');
