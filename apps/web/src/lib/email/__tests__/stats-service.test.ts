...
const mockCampaign = (id: string, name: string) =>
  vi.mocked(prisma.emailCampaign.findUnique).mockResolvedValue({ id, name } as any);

const mockStatusCounts = (counts: Array<{ status: string; count: number }>) =>
  vi.mocked(prisma.emailRecipient.groupBy).mockResolvedValue(
...
describe('getGlobalStats', () => {
  ...
  it('should filter by date range', async () => {
    ...
    vi.mocked(prisma.emailCampaign.findMany).mockResolvedValue([{ id: 'c1' }, { id: 'c2' }] as any);
    mockStatusCounts([
...
describe('getRecentCampaignStats', () => {
  ...
  it('should return last N campaigns with stats', async () => {
    ...
    vi.mocked(prisma.emailCampaign.findUnique).mockImplementation((({ where }: any) =>
