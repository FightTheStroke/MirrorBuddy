...
export async function saveMaterialsFromStudyKit(
  ...
  const existingToolIds = new Set(existingMaterials.map((m: { toolId: string }) => m.toolId));
  ...
  await prisma
    .$transaction(
      materials.map((material) =>
        prisma.material.upsert({
          where: { toolId: material.toolId, userId, mindmapRevision: null },
          create: material,
          update: {
    ...
    .catch((error: unknown) => {
      if (
        ...
        throw new ApiError(
          'Material is owned elsewhere or requires the mindmap command protocol',
          409,
        );
      }
      throw error;
 ...
 */
export async function deleteMaterialsFromStudyKit(studyKitId: string): Promise<number> {
