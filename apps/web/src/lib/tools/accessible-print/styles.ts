...
// Matches CSS hex colors, rgb()/rgba()/hsl()/hsla() functions, and bare
// alphabetic color keywords (e.g. "red", "steelblue"). Anything else
// (in particular anything containing `}`, `<`, `;`, or `url(`) is rejected,
// which prevents user-supplied accessibility settings from breaking out of
// the generated `<style>` block (CSS/HTML injection).
const SAFE_CSS_COLOR = /^(#[0-9a-fA-F]{3,8}|rgba?\([0-9.,%\s]+\)|hsla?\([0-9.,%\s]+\)|[a-zA-Z]+)$/;
...
export function getAccessibilityStyles(settings: Partial<AccessibilitySettings>): string {
  ...
  const backgroundColor = settings.highContrast
    ? '#000000'
    : sanitizeCssColor(settings.customBackgroundColor, '#ffffff');

  const textColor = settings.highContrast
    ? '#ffff00'
    : sanitizeCssColor(settings.customTextColor, '#1e293b');
