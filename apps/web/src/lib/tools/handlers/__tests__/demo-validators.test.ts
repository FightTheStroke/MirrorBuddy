...
describe("demo-validators", () => {
  ...
  describe("sanitizeHtml", () => {
    ...
    it("removes form tags", async () => {
      const html = '<form action="evil"><input></form>';
      const result = await sanitizeHtml(html);
      expect(result).not.toContain("form");
    });

    it("preserves classes and ids", async () => {
    ...
    it("handles null/undefined gracefully", async () => {
      expect(await sanitizeHtml(null as unknown as string)).toBe("");
      expect(await sanitizeHtml(undefined as unknown as string)).toBe("");
    });
  });
  ...
  describe("validateDescription", () => {
    ...
    it("suggests visual elements when missing", () => {
      ...
      expect(
        result.suggestions?.some((s) => s.includes("elemento visivo")),
      ).toBe(true);
    });
    ...
    it("suggests interaction type when missing", () => {
      ...
      expect(result.valid).toBe(false);
      expect(result.suggestions).toBeDefined();
      expect(result.suggestions?.some((s) => s.includes("interazione"))).toBe(
        true,
      );
    });
    ...
    it("suggests quantities when missing", () => {
      ...
      expect(result.valid).toBe(false);
      expect(result.suggestions).toBeDefined();
      expect(result.suggestions?.some((s) => s.includes("quantità"))).toBe(
        true,
      );
    });
    ...
    it("recognizes various interaction keywords", () => {
      ...
      for (const keyword of interactionKeywords) {
        ...
        expect(
          result.suggestions?.some((s) => s.includes("tipo di interazione")),
        ).toBeFalsy();
      }
    });
