...
describe('tool-persistence', () => {
  ...
  const createMockMaterial = (overrides: Partial<MaterialRecord> = {}): MaterialRecord => ({
    id: 'mat-1',
    toolId: 'tool-123',
    userId,
    toolType: 'mindmap',
  ...
  describe('saveTool', () => {
    ...
    it('should create derived_from edge when sourceToolId provided', async () => {
      ...
      vi.mocked(prisma.material.create).mockResolvedValue(mockMaterial as any);
      vi.mocked(prisma.material.findUnique).mockResolvedValue(sourceMaterial as any);
  ...
  describe('getToolById', () => {
    it('should return tool by id or toolId', async () => {
      const mockMaterial = createMockMaterial();
      vi.mocked(prisma.material.findFirst).mockResolvedValue(mockMaterial as any);
