...
`;
        ...
        expect(result.success).toBe(true);
        const hasQuote = result.mindmap?.root.children?.some((c) =>
          c.text.includes("quote"),
        );
  ...
  <node TEXT="Root" COLOR="#ff0000">
    <node TEXT="Child" COLOR="#00ff00"/>
      ...
      it("should fail for invalid XML", async () => {
        ...
        const result = await importMindmap(xml, "bad.mm", {
          format: "freemind",
        });
      ...
      it("should fail for XML without map element", async () => {
        ...
        const result = await importMindmap(xml, "nomap.mm", {
          format: "freemind",
        });
...
`;
  ...
  describe("validateMindmap", () => {
    it("should validate correct mindmap structure", () => {
      const mindmap: MindmapData = {
        ...
        root: {
          id: "root-1",
          text: "Root",
          children: [{ id: "child-1", text: "Child" }],
        },
      };
    ...
    it("should validate nested children", () => {
      ...
      expect(result.valid).toBe(false);
      expect(
        result.errors.some((e) => e.includes("children[0].children[0]")),
      ).toBe(true);
    });
  });
});
