    ...
    <>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList className="grid w-full grid-cols-4">
          <TabsTrigger value="dashboard">{t('stripe.tabs.dashboard') ?? 'Dashboard'}</TabsTrigger>
          <TabsTrigger value="products">{t('stripe.tabs.products') ?? 'Products'}</TabsTrigger>
          <TabsTrigger value="subscriptions">
            {t('stripe.tabs.subscriptions') ?? 'Subscriptions'}
          </TabsTrigger>
          <TabsTrigger value="webhooks">{t('stripe.tabs.webhooks') ?? 'Webhooks'}</TabsTrigger>
        </TabsList>

        <TabsContent value="dashboard" className="mt-6">
          <StripeDashboardTab dashboard={dashboard} initialSettings={settings} />
        </TabsContent>
