...
describe('ProtectedUsersCard', () => {
  ...
  it('renders card with title and description', () => {
    ...
    expect(screen.getByText('Utenti esclusi da cleanup test')).toBeInTheDocument();
  });
  ...
  it('displays empty state message when PROTECTED_USERS is not set', () => {
    ...
    expect(screen.getByText(getTranslation('admin.protectedUsers.empty'))).toBeInTheDocument();
  });
  ...
  it('trims whitespace from email addresses', () => {
    process.env.PROTECTED_USERS = 'user1@example.com , user2@example.com , user3@example.com';
    render(<ProtectedUsersCard />);
  ...
  it('handles emails with special characters', () => {
    process.env.PROTECTED_USERS = 'user+tag@example.com,user.name@sub.example.com';
    render(<ProtectedUsersCard />);
