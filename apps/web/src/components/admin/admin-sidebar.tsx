...
import { PanelLeftClose, PanelLeftOpen, Home, ChevronDown, ChevronRight } from 'lucide-react';
    ...
    >
      ...
      <div className="h-14 flex items-center justify-between px-4 border-b border-slate-200 dark:border-slate-800">
        <Link href="/admin" className="flex items-center gap-3 hover:opacity-80 transition-opacity">
          ...
          />
          {open && (
            <span className="font-bold text-lg text-slate-900 dark:text-white">{t('admin')}</span>
          )}
        </Link>
        <Button
          variant="ghost"
      ...
      <nav className="p-4 space-y-3 overflow-y-auto" style={{ maxHeight: 'calc(100vh - 120px)' }}>
