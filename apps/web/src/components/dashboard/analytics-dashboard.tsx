    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          value={
            azureCosts?.configured === false ? 'N/D' : `$${(azureCosts?.totalCost || 0).toFixed(2)}`
          }
          subtext={azureCosts?.configured === false ? t('notConfigured') : 'Azure + AI'}
          color="amber"
        />
      </div>
                  ...
                  <div key={id} className="space-y-1">
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <span className="text-muted-foreground">
                        {data.count} {data.count === 1 ? t('sessionSingular') : t('sessionPlural')}
                      </span>
                    </div>
                    <div className="h-2 bg-muted rounded-full overflow-hidden">
                      <motion.div
                        ...
                        style={{
                          backgroundColor: subjectColors[data.subject as Subject] || '#6366f1',
                        }}
                        initial={{ width: 0 }}
            ...
            {!tokenData || Object.keys(tokenData.byAction).length === 0 ? (
              ...
              Object.entries(tokenData.byAction).map(([action, data]) => {
                const maxTokens = Math.max(
                  ...Object.values(tokenData.byAction).map((d) => d.totalTokens),
                );
        ...
        <Card>
          ...
          <CardContent>
            {azureCosts?.configured === false ? (
              ...
              <p className="text-sm text-muted-foreground text-center py-8">{t('noCostData')}</p>
            )}
          </CardContent>
        </Card>
      </div>
