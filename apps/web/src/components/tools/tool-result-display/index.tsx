...
export function ToolResultDisplay({
  ...
  const icon = toolIcons[toolCall.type] || toolIcons[toolType] || (
  ...
  return (
    <motion.div
      ...
      className={cn(
        "space-y-2 w-full",
        isFullscreen &&
          "fixed inset-0 z-50 bg-white dark:bg-slate-950 p-6 overflow-auto",
        className,
      )}
      role="region"
    ...
    >
      ...
      <div
        className={cn(
          ...
          isFullscreen &&
            "sticky top-0 z-10 bg-white dark:bg-slate-950 pb-4 border-b border-slate-200 dark:border-slate-700 mb-4",
        )}
      >
        <div className="flex items-center gap-2">
          <span
            className={cn(
              "text-slate-400",
              isFullscreen && "text-slate-600 dark:text-slate-300",
            )}
          >
            {icon}
          </span>
          <span
            className={cn(
              ...
              isFullscreen && "text-slate-900 dark:text-white text-lg",
            )}
          >
            {name}
          </span>
