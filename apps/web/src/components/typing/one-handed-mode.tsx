...
export function OneHandedMode({ currentMode, onModeChange, disabled = false }: OneHandedModeProps) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-semibold" id="one-handed-title">
          {t('title')}
        </h3>
        <button
      ...
      <div
        ...
        role="group"
        aria-labelledby="one-handed-title"
      >
        {modes.map((mode) => (
          <button
            key={mode.value}
