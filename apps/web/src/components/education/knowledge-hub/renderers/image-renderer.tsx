 ...
 *   title?: string;
 *   url?: string;
 *   imageData?: string; // Legacy archive photo
 *   imageBase64?: string; // Persisted webcam photo
 *   caption?: string;
 *   alt?: string;
 * }
 */
    ...
    <>
      ...
      >
        <div className="relative group">
          ...
          <img src={url} alt={alt} className="w-full h-auto object-contain max-h-96" />
        ...
        {(title || imageData.caption) && (
          <div className="p-3 border-t border-slate-200 dark:border-slate-700">
            {title && <h4 className="font-medium text-slate-900 dark:text-slate-100">{title}</h4>}
            {imageData.caption && (
              <p className="text-sm text-slate-600 dark:text-slate-400">{imageData.caption}</p>
            )}
          </div>
        )}
      </motion.div>
