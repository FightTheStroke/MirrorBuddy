...
export function StudyKitViewer({
  studyKit,
  onDelete,
  onGeneratePath,
  className,
  ...
  const demoCode = useMemo(
    () => (studyKit.demo ? buildDemoCode(studyKit.demo) : null),
    [studyKit.demo],
  );
  const baseQuiz = useMemo(
    () => (studyKit.quiz ? transformQuizData(studyKit.quiz, studyKit) : null),
    [studyKit],
  );
  const transformedQuiz = useMemo(
    () =>
      studyKit.quiz
        ? transformQuizData(studyKit.quiz, studyKit, adaptiveDifficulty ?? undefined)
        : null,
  ...
  const handleQuizComplete = useCallback(
    (result: QuizResult) => {
      ...
      const avgDifficulty =
        transformedQuiz.questions.reduce((sum, q) => sum + q.difficulty, 0) /
        transformedQuiz.questions.length;
  ...
  const materialCount = [studyKit.summary, studyKit.mindmap, studyKit.demo, studyKit.quiz].filter(
    Boolean,
  ).length;
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            ...
            {studyKit.summary ? (
              <div
                ...
                dangerouslySetInnerHTML={{
                  __html: sanitize(`<p class="mb-3">${parsedSummary}</p>`),
                }}
              />
            ) : (
              <p className="text-slate-500 text-center py-8">{t('riassuntoNonDisponibile')}</p>
            )}
          </div>
        </TabsContent>
        ...
        <TabsContent value="mindmap" className="mt-6">
          {studyKit.mindmap ? (
            ...
            <div className="bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 p-6">
              <p className="text-slate-500 text-center py-8">{t('mappaMentaleNonDisponibile')}</p>
            </div>
          )}
        </TabsContent>
                ...
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    ...
                    <span className="text-sm text-slate-500">
                      {studyKit.demo &&
                      'title' in studyKit.demo &&
                      typeof studyKit.demo.title === 'string'
                        ? studyKit.demo.title
        ...
        <TabsContent value="quiz" className="mt-6">
          {transformedQuiz ? (
            ...
            <div className="bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 p-6">
              <p className="text-slate-500 text-center py-8">{t('quizNonDisponibile')}</p>
            </div>
          )}
        </TabsContent>
      </Tabs>
