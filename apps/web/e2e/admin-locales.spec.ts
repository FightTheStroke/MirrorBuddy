...
test.describe('Admin Locales Management', () => {
  ...
  test.describe('Locale List Display', () => {
    test('should display locales page with title and create button', async ({ adminPage }) => {
      // Verify page heading
      await expect(
        adminPage.locator('h1, h2').filter({ hasText: /Gestione Lingue|Lingue e Paesi|Locales/i }),
      ).toBeVisible();
    ...
    test('should display locale list table with all columns', async ({ adminPage }) => {
      // Verify table headers are present
      await expect(adminPage.locator("thead th:has-text('Codice Paese')")).toBeVisible();
      await expect(adminPage.locator("thead th:has-text('Nome Paese')")).toBeVisible();
      await expect(adminPage.locator("thead th:has-text('Locale Primario')")).toBeVisible();
      await expect(adminPage.locator("thead th:has-text('Maestro Lingua')")).toBeVisible();
      await expect(adminPage.locator("thead th:has-text('Locali Secondari')")).toBeVisible();
      await expect(adminPage.locator("thead th:has-text('Stato')")).toBeVisible();
      await expect(adminPage.locator("thead th:has-text('Azioni')")).toBeVisible();
    });

    test('should display empty state when no locales exist', async ({ adminPage }) => {
      ...
      await expect(adminPage.locator('text=Nessuna configurazione locale trovata')).toBeVisible();
    });
    ...
    test('should filter locales by search query', async ({ adminPage }) => {
      ...
      // Get search input
      const searchInput = adminPage.locator('input[placeholder*="Cerca per codice paese"]');
    ...
    test('should display locale status badge (active/inactive)', async ({ adminPage }) => {
  ...
  test.describe('Create New Locale Configuration', () => {
    test('should navigate to create page when clicking create button', async ({ adminPage }) => {
      const createButton = adminPage
        .locator('button, a')
        .filter({ hasText: /Nuova Configurazione|Crea|New/i })
        .first();
    ...
    test('should display locale creation form with required fields', async ({ adminPage }) => {
      ...
      await expect(
        adminPage.locator('input[name="countryName"], input[placeholder*="Nome Paese"]'),
      ).toBeVisible();
      await expect(
        adminPage.locator('input[name="primaryLocale"], input[placeholder*="Locale Primario"]'),
      ).toBeVisible();
      await expect(
        adminPage.locator('input[name="primaryLanguageMaestroId"], input[placeholder*="Maestro"]'),
      ).toBeVisible();
      ...
      await expect(
        adminPage.locator('button, a').filter({ hasText: /Annulla|Indietro|Cancel/i }),
      ).toBeVisible();
    });
    ...
    test('should create new locale with API mock', async ({ adminPage }) => {
      ...
      await adminPage.locator('input[name="primaryLanguageMaestroId"]').fill('moliere');

      // Submit form
      const submitButton = adminPage.locator('button').filter({ hasText: /Salva|Crea|Submit/i });
    ...
    test('should return to locales list when clicking cancel', async ({ adminPage }) => {
      await adminPage.goto('/admin/locales/new');
      await adminPage.waitForLoadState('domcontentloaded');
    ...
    test('should show validation errors for invalid input', async ({ adminPage }) => {
      await adminPage.goto('/admin/locales/new');
      await adminPage.waitForLoadState('domcontentloaded');
  ...
  test.describe('Edit Existing Locale Configuration', () => {
    test('should navigate to edit page when clicking edit button', async ({ adminPage }) => {
      // Mock locale list
      await adminPage.route('**/api/admin/locales', async (route) => {
        if (route.request().method() === 'GET') {
          await route.fulfill({
    ...
    test('should display edit form with pre-filled values', async ({ adminPage }) => {
      ...
      await expect(
        adminPage.locator('h1, h2').filter({ hasText: /Modifica|Edit|Aggiorna/i }),
      ).toBeVisible();

      // Verify fields are pre-filled
      await expect(adminPage.locator('input[name="id"], input[value="it"]')).toHaveValue('it');
      await expect(
      ...
      await expect(
        adminPage.locator('input[name="primaryLanguageMaestroId"], input[value="manzoni"]'),
      ).toHaveValue('manzoni');
    });

    test('should update locale configuration with API mock', async ({ adminPage }) => {
  ...
  test.describe('Delete Locale Configuration', () => {
    test('should show delete confirmation dialog', async ({ adminPage }) => {
      ...
      if (await deleteButton.isVisible({ timeout: 1000 }).catch(() => false)) {
        ...
        await expect(adminPage.locator("[role='dialog'], .modal, .alert").first()).toBeVisible();
      }
    });
  ...
  test.describe('Locale Preview Functionality', () => {
    test('should display preview button for locale greetings', async ({ adminPage }) => {
    ...
    test('should show greeting preview in correct language', async ({ adminPage }) => {
      ...
      if (await previewSection.isVisible({ timeout: 1000 }).catch(() => false)) {
  ...
  test.describe('Audit Log Entries', () => {
    test('should create audit log entry on locale creation', async ({ adminPage }) => {
      // Mock API responses
      await adminPage.route('**/api/admin/locales', async (route) => {
        if (route.request().method() === 'POST') {
          await route.fulfill({
      ...
      await adminPage.locator('input[name="primaryLanguageMaestroId"]').fill('cervantes');
    ...
    test('should create audit log entry on locale update', async ({ adminPage }) => {
      // Mock endpoints
      await adminPage.route('**/api/admin/locales/it', async (route) => {
        if (route.request().method() === 'GET') {
          await route.fulfill({
    ...
    test('should create audit log entry on locale deletion', async ({ adminPage }) => {
      // Mock DELETE request
      await adminPage.route('**/api/admin/locales/*', async (route) => {
        if (route.request().method() === 'DELETE') {
          await route.fulfill({
