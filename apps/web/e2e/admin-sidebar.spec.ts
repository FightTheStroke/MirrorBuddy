...
test.describe('Admin Sidebar Navigation', () => {
  ...
  test('F-01: clicking each sidebar link navigates correctly', async ({ adminPage }) => {
    ...
    for (const navItem of ADMIN_NAV_ITEMS) {
      ...
      await Promise.all([
        adminPage.waitForURL(
          (url) =>
            navItem.exact ? url.pathname === expectedPath : url.pathname.startsWith(expectedPath),
          { timeout: 10000 },
        ),
  ...
  test("F-03: 'Torna all\\'app' button navigates to home", async ({ adminPage }) => {
    await dismissBlockingModals(adminPage);
    ...
    await expect(tornaButton, "'Torna all'app' button should be visible").toBeVisible();
    ...
    expect(isValidDestination, `Should go to main app but went to "${pathname}"`).toBe(true);
    expect(navigationTo404, 'Should not result in 404').toBe(false);
    expect(pathname.startsWith('/admin'), `Should navigate away from admin`).toBe(false);
  });
  ...
  test('F-04: sidebar collapse/expand toggle works', async ({ adminPage }) => {
    ...
    if (await toggleButton.isVisible().catch(() => false)) {
      ...
      expect(newWidth, 'Sidebar width should change after toggle').not.toBe(initialWidth);
  ...
  test('F-05: admin logo links to admin dashboard', async ({ adminPage }) => {
    ...
    const logoLink = adminPage.locator('aside').locator('a[href="/admin"]').first();
  ...
  test('F-02: no 404 responses during admin navigation', async ({ adminPage }) => {
    await dismissBlockingModals(adminPage);
    ...
    for (const navItem of ADMIN_NAV_ITEMS) {
      const link = adminPage.locator('aside').locator('a', { hasText: navItem.label }).first();

      if (await link.isVisible({ timeout: 1000 }).catch(() => false)) {
        await Promise.all([
          adminPage.waitForURL(
            (url) =>
              navItem.exact ? url.pathname === navItem.href : url.pathname.startsWith(navItem.href),
            { timeout: 10000 },
          ),
    ...
    expect(notFoundUrls, `404 errors found: ${notFoundUrls.join(', ')}`).toHaveLength(0);
  });
});
