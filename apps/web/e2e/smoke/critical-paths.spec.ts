...
test.describe('SMOKE: Critical Paths @smoke', () => {
  ...
  test('CP-01: Welcome page loads without JavaScript errors', async ({ page }) => {
    ...
    expect(navigated, 'Failed to navigate to /it/welcome after 2 attempts').toBe(true);
  ...
  test('CP-02: Home page loads for authenticated users', async ({ page }) => {
    ...
    const criticalErrors = errors.filter(
      (e) => !e.includes('ResizeObserver') && !e.includes('Non-Error') && !e.includes('hydration'),
    );
  ...
  test('CP-05: Admin dashboard requires authentication', async ({ page }) => {
    ...
    const loginOrRedirected = await page
      .waitForURL((url) => url.pathname.includes('/login') || !url.pathname.includes('/admin'), {
        timeout: 8000,
      })
      .then(() => true)
      .catch(() => false);
    ...
    const currentUrl = page.url();
    const isOnLogin = currentUrl.includes('/login') || /\/[a-z]{2}\/login/.test(currentUrl);
    ...
    expect(
      loginOrRedirected || isOnLogin || redirectedAwayFromAdmin || hasUnauthorized,
  ...
  test('CP-06: Logout clears session and redirects', async ({ page }) => {
    ...
    const hasAuthCookie = cookiesBefore.some((c) => c.name === 'mirrorbuddy-user-id');
  ...
  test('CP-07: Static assets load correctly (no i18n redirect)', async ({ request }) => {
