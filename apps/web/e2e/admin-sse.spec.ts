...
test.describe('Admin SSE - Real-time Dashboard Updates', () => {
  test('F-08, F-20: SSE connection establishes and sends initial data', async ({ adminPage }) => {
  ...
  test('F-21: SSE heartbeat messages keep connection alive', async ({ adminPage }) => {
    // Mock ToS API
    await adminPage.route('/api/tos', async (route) => {
      await route.fulfill({
        status: 200,
  ...
  test('F-20: Initial data sent within 500ms (performance target)', async ({ adminPage }) => {
    // Mock ToS API
    await adminPage.route('/api/tos', async (route) => {
      await route.fulfill({
        status: 200,
