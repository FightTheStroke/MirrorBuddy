...
test.describe('Mission Control API - Authentication', () => {
  test('all mission control APIs return 401 for unauthenticated requests', async ({
    ...
    for (const api of MISSION_CONTROL_APIS) {
      ...
      expect(response.status(), `${api.path} should return 401 for unauthenticated request`).toBe(
        401,
      );
...
test.describe('Mission Control API - Authenticated Access', () => {
  test('key-vault API returns secrets or server error for admin', async ({ adminRequest }) => {
  ...
  test('health-aggregator API returns health data for admin', async ({ adminRequest }) => {
  ...
  test('stripe API returns subscription data for admin', async ({ adminRequest }) => {
  ...
  test('ops-dashboard API returns metrics for admin', async ({ adminRequest }) => {
  ...
  test('infra-panel API returns infrastructure data for admin', async ({ adminRequest }) => {
  ...
  test('ai-email API returns AI/email metrics for admin', async ({ adminRequest }) => {
  ...
  test('business-kpi API returns KPI data for admin', async ({ adminRequest }) => {
  ...
  test('control-panel API returns control settings for admin', async ({ adminRequest }) => {
  ...
  test('grafana API returns Grafana config for admin', async ({ adminRequest }) => {
...
test.describe('Mission Control API - Data Structure Validation', () => {
  test('key-vault API returns masked secrets with expected fields', async ({ adminRequest }) => {
    const response = await adminRequest.get('/api/admin/key-vault');
    // key-vault may return 500 if encryption keys are not available (test env)
    expect([200, 500]).toContain(response.status());
...
test.describe('Mission Control API - Error Handling', () => {
  test('APIs handle malformed requests gracefully', async ({ adminRequest }) => {
