...
test.describe('Settings Page Interactions', () => {
  ...
  test.describe('Navigation and Access', () => {
    test('settings page is accessible via sidebar navigation', async ({ trialPage: page }) => {
      await page.goto('/');
      ...
      if (!isSettingsVisible) {
        ...
        const settingsIcon = page.locator('[aria-label*="settings" i], button:has(svg)').first();
        const hasSettingsIcon = await settingsIcon.isVisible().catch(() => false);

        if (!hasSettingsIcon) {
          // Skip test if no settings access for trial users
          console.log('Settings button not visible for trial user - test skipped');
          return;
        }
    ...
    test('settings page loads without 403 errors', async ({ trialPage: page }) => {
  ...
  test.describe('Interactive Elements', () => {
      ...
      if (await settingsBtn.isVisible()) {
        ...
        if (count > 0) {
          ...
          if (await firstCharacter.isVisible()) {
            ...
            const isAriaChecked = await firstCharacter.getAttribute('aria-checked');
            const isDataSelected = await firstCharacter.getAttribute('data-selected');
    ...
    test('theme toggle is interactive', async ({ trialPage: page }) => {
      ...
      if (await settingsBtn.isVisible()) {
        ...
        const themeToggle = page
          .locator('[role="switch"]:has-text(/Scuro|Chiaro|Dark|Light/i), button:has-text(/Tema/i)')
          .first();
    ...
    test('accessibility panel is present and functional', async ({ trialPage: page }) => {
      await page.goto('/');
      await page.waitForLoadState('domcontentloaded');
      ...
      if (await settingsBtn.isVisible()) {
        ...
        const a11ySection = page
          .locator('[data-testid="a11y"], text=/Accessibilità/i, button:has-text(/Accessibilità/i)')
          .first();

        if (await a11ySection.isVisible()) {
          // Click to open/expand if needed
          const isButton = await a11ySection.evaluate((el) => el.tagName === 'BUTTON');
          if (isButton) {
  ...
  test.describe('Settings Persistence', () => {
    test('character selection persists after refresh', async ({ trialPage: page }) => {
      await page.goto('/');
      await page.waitForLoadState('domcontentloaded');
  ...
  test.describe('Error Handling', () => {
    test('no critical console errors in settings', async ({ trialPage: page }) => {
