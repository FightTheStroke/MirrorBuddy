...
import { waitForPageReady, checkLayoutStability } from './admin-visual-regression-helpers';
...
test.describe('Parent Dashboard Charts - Very Small Mobile (xs < 375px)', () => {
  ...
  test('quiz performance - single column on very small screens', async ({ page }) => {
    ...
    expect(bodyWidth, `Content wider than viewport: ${bodyWidth} > 320`).toBeLessThanOrEqual(330);
  });

  test('parent dashboard - no horizontal scroll on very small mobile', async ({ page }) => {
    await page.goto('/profile');
    await waitForPageReady(page);
...
test.describe('Parent Dashboard Charts - Mobile (375px - 640px)', () => {
  ...
  test('quiz performance - stats cards readable on mobile', async ({ page }) => {
    await page.goto('/profile');
    await waitForPageReady(page);
  ...
  test('parent dashboard - no horizontal overflow on mobile', async ({ page }) => {
    await page.goto('/profile');
    await waitForPageReady(page);

    const layoutIssues = await checkLayoutStability(page);
    expect(layoutIssues, `Mobile layout issues: ${layoutIssues.join(', ')}`).toHaveLength(0);
  ...
  test('subject studied - progress bars readable on mobile', async ({ page }) => {
    await page.goto('/profile');
    await waitForPageReady(page);
...
test.describe('Parent Dashboard Charts - Tablet (640px - 1024px)', () => {
  ...
  test('parent dashboard charts - responsive layout on tablet', async ({ page }) => {
    await page.goto('/profile');
    await waitForPageReady(page);

    const layoutIssues = await checkLayoutStability(page);
    expect(layoutIssues, `Tablet layout issues: ${layoutIssues.join(', ')}`).toHaveLength(0);
  });
  ...
  test('quiz performance - 2-3 columns on tablet', async ({ page }) => {
    ...
    const quizStatsGrid = await page.locator("[aria-label='Performance quiz'] >> div.grid").first();
...
test.describe('Parent Dashboard Charts - Desktop (1024px+)', () => {
  ...
  test('parent dashboard charts - full responsive layout on desktop', async ({ page }) => {
    await page.goto('/profile');
    await waitForPageReady(page);

    const layoutIssues = await checkLayoutStability(page);
    expect(layoutIssues, `Desktop layout issues: ${layoutIssues.join(', ')}`).toHaveLength(0);
  });
...
test.describe('Parent Dashboard Charts - Touch Accessibility', () => {
  ...
  test('data points and interactive elements are touch-friendly', async ({ page }) => {
    await page.goto('/profile');
    await waitForPageReady(page);
  ...
  test('legend elements are repositioned on mobile', async ({ page }) => {
    ...
    if (badges.length > 0) {
      ...
      const container = await page.locator("[aria-label='Materie studiate']").first();
