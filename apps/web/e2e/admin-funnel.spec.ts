...
test.describe('Admin Funnel Dashboard', () => {
  ...
  test('F-05: funnel dashboard page loads successfully', async ({ adminPage }) => {
    ...
    expect(errors.filter((e) => !ADMIN_IGNORE_ERRORS.some((p) => p.test(e)))).toHaveLength(0);
  });
...
test.describe('Admin Funnel API', () => {
  test('F-03: funnel metrics API returns data', async ({ adminPage }) => {
    ...
    const response = await adminPage.request.get('/api/admin/funnel/metrics?days=30');
  ...
  test('F-03: funnel users API returns data', async ({ adminPage }) => {
    const response = await adminPage.request.get('/api/admin/funnel/users?page=1&pageSize=10');
  ...
  test('F-03: funnel metrics API requires auth', async ({ page }) => {
    ...
    const response = await page.request.get('/api/admin/funnel/metrics?days=30');
