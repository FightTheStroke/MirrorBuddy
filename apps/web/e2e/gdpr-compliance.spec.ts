...
test.describe('GDPR Compliance: User Data Deletion (Art. 17)', () => {
  ...
  test('GET /api/privacy/delete-my-data - returns 401 without session', async ({ request }) => {
  ...
  test('POST /api/privacy/delete-my-data - requires confirmation', async ({ request }) => {
    await request.get('/api/user');
    const csrfToken = await getCsrfToken(request);
  ...
  test('POST /api/privacy/delete-my-data - deletes user data completely', async ({ request }) => {
    // Create fresh user
...
test.describe('GDPR Compliance: Data Retention Policy', () => {
  test('GET /api/user/settings - includes retention preferences', async ({ request }) => {
    await request.get('/api/user');
  ...
  test('Conversations can be soft-deleted', async ({ request }) => {
    ...
    // Delete conversation
    const deleteResponse = await request.delete(`/api/conversations/${conv.id}`, {
      headers: { 'x-csrf-token': csrfToken },
    });
    expect(deleteResponse.ok()).toBeTruthy();
...
test.describe('GDPR Compliance: Data Minimization', () => {
  ...
  test('Learnings API supports pagination (limits data exposure)', async ({ request }) => {
    await request.get('/api/user');
...
test.describe('GDPR Compliance: Consent & Transparency', () => {
  test('Health endpoint returns privacy-compliant status', async ({ request }) => {
