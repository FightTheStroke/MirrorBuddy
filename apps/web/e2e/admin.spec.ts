...
test.describe('Admin Mode - Login Flow', () => {
  test('F-12: admin login flow works with .env credentials', async ({ page }) => {
    const errors: string[] = [];
    page.on('console', (msg) => {
      if (msg.type() === 'error') {
        const text = msg.text();
...
test.describe('Admin Mode - Route Accessibility', () => {
  test('F-01,02,04,05,10,13,34: all admin routes accessible', async ({ adminPage }) => {
    ...
    if (auditReport.length > 0) {
      auditReport.forEach((issue) => {
        console.log(`  [${issue.severity.toUpperCase()}] ${issue.route}: ${issue.message}`);
      });
    }
  ...
  test('F-07: audit ignores chat and voice endpoints', async ({ adminPage }) => {
    ...
    const chatPattern = ADMIN_IGNORE_ERRORS.find((p) => p.source.includes('chat'));
    const voicePattern = ADMIN_IGNORE_ERRORS.find((p) => p.source.includes('voice'));
