...
test.describe('Parent Dashboard Route (F-16)', () => {
  test('authenticated user can access /it/parent-dashboard', async ({ adminPage }) => {
  ...
  test('parent dashboard page loads in English locale', async ({ adminPage }) => {
  ...
  test('parent dashboard page title is set correctly', async ({ adminPage }) => {
    await adminPage.goto('/it/parent-dashboard');
    await adminPage.waitForLoadState('domcontentloaded');
  ...
  test('parent dashboard does not throw JavaScript errors on load', async ({ adminPage }) => {
    const errors: string[] = [];
    ...
    const criticalErrors = errors.filter(
      (err) =>
        !err.includes('ResizeObserver') && !err.includes('Warning') && !err.includes('hydration'),
    );
