...
test.describe('Admin Component Consistency (F-01)', () => {
  test('all pages use consistent button styling', async ({ adminPage }) => {
    ...
    for (const route of routesToTest) {
      ...
      if (buttonCount > 0) {
        ...
        expect(styles.padding, `Button padding consistent on ${route}`).toBeTruthy();
        expect(styles.borderRadius, `Button border-radius consistent on ${route}`).toBeTruthy();
      }
    }
  });
  ...
  test('all pages use consistent spacing grid', async ({ adminPage }) => {
    ...
    for (const route of routesToTest) {
      ...
      expect(gridCount, `Grid spacing utilities found on ${route}`).toBeGreaterThan(0);
    }
  });
  ...
  test('headings and structure consistent', async ({ adminPage }) => {
    ...
    for (const route of routesToTest) {
      ...
      expect(await mainHeading.count(), `Main heading present on ${route}`).toBeGreaterThan(0);

      // Check for semantic structure
      const semantic = adminPage.locator('main, nav, aside, section');
      expect(await semantic.count(), `Semantic HTML found on ${route}`).toBeGreaterThan(0);
      ...
      if ((await lists.count()) > 0) {
        ...
        expect(await listItems.count(), `List items found on ${route}`).toBeGreaterThan(0);
      }
    }
  });
  ...
  test('interactive elements are accessible', async ({ adminPage }) => {
    ...
    for (const route of routesToTest) {
      ...
      const interactiveElements = adminPage.locator('button, a, input, select, textarea');
      if ((await interactiveElements.count()) > 0) {
        const focusOutline = await interactiveElements.first().evaluate((el) => {
          return window.getComputedStyle(el).outline;
        });
        // Should have some form of focus indicator (outline, ring, etc)
        expect(
          focusOutline ||
            (await interactiveElements.first().locator("[class*='focus']").count()) > 0,
        ).toBeTruthy();
      }
    }
  });
