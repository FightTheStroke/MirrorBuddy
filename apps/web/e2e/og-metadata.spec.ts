...
test.describe('Open Graph Metadata', () => {
  for (const locale of locales) {
    ...
    test(`should have og:locale:alternate for ${locale}`, async ({ page }) => {
      ...
      const alternates = await page.locator('meta[property="og:locale:alternate"]').all();

      // Should have 4 alternates (all other locales)
      expect(alternates.length).toBe(4);

      // Get all alternate values
      const alternateValues = await Promise.all(alternates.map((el) => el.getAttribute('content')));
  ...
  test('should have consistent metadata across all locales', async ({ page }) => {
