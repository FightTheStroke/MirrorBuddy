...
test.describe('Google SSO', () => {
  test('SSO initiation endpoint returns redirect', async ({ request }) => {
    const response = await request.get('/api/auth/sso/google', {
      maxRedirects: 0,
    });
  ...
  test('SSO callback rejects missing code parameter', async ({ request }) => {
    const response = await request.get('/api/auth/sso/google/callback', {
      maxRedirects: 0,
    });
    expect([302, 307, 308]).toContain(response.status());
  ...
  test('SSO callback rejects invalid state', async ({ request }) => {
    const response = await request.get('/api/auth/sso/google/callback?code=test&state=invalid', {
      maxRedirects: 0,
    });
    expect([302, 307, 308]).toContain(response.status());
    const location = response.headers()['location'] || '';
  ...
  test('SSO callback handles OAuth error parameter', async ({ request }) => {
    const response = await request.get('/api/auth/sso/google/callback?error=access_denied', {
      maxRedirects: 0,
    });
    expect([302, 307, 308]).toContain(response.status());
    const location = response.headers()['location'] || '';
