...
test.describe('Compliance Pages - Content Verification', () => {
  test('privacy policy contains privacy commitment', async ({ page }) => {
    ...
    const hasPrivacyContent = /data|privacy|protection|gdpr|dati|protezione/i.test(content!);
  ...
  test('AI transparency page has required sections', async ({ page }) => {
    ...
    const content = await page.textContent('body');
    expect(content).toBeTruthy();
    const hasAIContent = /AI|intelligenza artificiale|azure|openai|trasparenza/i.test(content!);
    expect(hasAIContent).toBeTruthy();
  });

  test('compliance pages have page headers', async ({ page }) => {
    test.setTimeout(60000); // 4 page navigations
    const pages = ['/it/privacy', '/it/terms', '/it/cookies', '/it/ai-transparency'];
    ...
    for (const pagePath of pages) {
      ...
      expect(isVisible, `Page ${pagePath} should have a visible heading`).toBe(true);
    }
  });
});
...
test.describe('Compliance Pages - Keyboard Navigation', () => {
  ...
  test('AI transparency page links are keyboard accessible', async ({ page }) => {
    await page.goto('/it/ai-transparency');
    await page.waitForLoadState('domcontentloaded');
...
test.describe('Compliance Pages - Multi-Locale', () => {
  ...
  for (const compliancePage of compliancePages) {
    testAllLocales(`${compliancePage.name} loads in all locales`, async ({ localePage }) => {
...
test.describe('Compliance Pages - Country-Specific Content', () => {
  ...
  for (const [country, config] of Object.entries(countryMappings)) {
    test(`accessibility page shows correct authority for ${country}`, async ({ page }) => {
    ...
    test(`cookie consent shows correct language for ${country}`, async ({ page }) => {
