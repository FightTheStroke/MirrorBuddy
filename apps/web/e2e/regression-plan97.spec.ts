...
test.describe('Regression: TosGateProvider Blocking (Plan 97)', () => {
  ...
  test('should allow page interaction when /api/tos is mocked', async ({ page }) => {
...
test.describe('Regression: Missing i18n Compliance Keys (Plan 97)', () => {
  test('should not have missing translation errors in compliance pages', async ({ page }) => {
    const testPages = ['/it/privacy', '/it/cookies', '/it/accessibility', '/it/ai-transparency'];
  ...
  testAllLocales('should have all compliance sections translated', async ({ localePage }) => {
    await localePage.goto('/accessibility');
    await localePage.page.waitForLoadState('domcontentloaded');

    const mainContent = localePage.page.locator("main, [role='main']").first();
    await expect(mainContent).toBeVisible({ timeout: 10000 });
...
test.describe('Regression: Welcome Page LocalePage Pattern (Plan 97)', () => {
  testAllLocales('should use localePage.goto() pattern correctly', async ({ localePage }) => {
    await localePage.goto('/welcome');

    expect(localePage.page.url()).toContain(`/${localePage.locale}/welcome`);
    expect(await localePage.page.locator('html').getAttribute('lang')).toBe(localePage.locale);

    await expect(localePage.page.locator("main, [role='main']").first()).toBeVisible({
      timeout: 10000,
    });
  });
...
test.describe('Regression: Protected Routes in Tests (Plan 97)', () => {
  test('should avoid testing protected routes without auth', async ({ browser }) => {
    ...
    for (const route of protectedRoutes) {
      ...
      const hasRedirected = currentUrl.includes('/login') || currentUrl.includes('/welcome');
...
test.describe('Regression: Overall E2E Stability (Plan 97)', () => {
  ...
  test('should load and interact with welcome page without blocking', async ({ page }) => {
    await page.goto('/it/welcome');
    await page.waitForLoadState('domcontentloaded');
  ...
  test('should load compliance pages across locales without errors', async ({ page }) => {
    const testCases = [
