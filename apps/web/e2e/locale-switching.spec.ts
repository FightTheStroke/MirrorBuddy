...
import { localePatterns, verifyPageLocale, waitForLocale, buildLocalizedPath } from './fixtures';
...
test.describe('Locale Switching and Content Localization', () => {
   ...
   */
  test('user can switch language via URL navigation', async ({ localePage }) => {
   ...
   */
  test('language preference persists across pages', async ({ localePage, context }) => {
    ...
    for (const page of pagesToTest) {
      ...
      const verification = await verifyPageLocale(localePage.page, targetLocale);
      expect(verification.isValid).toBeTruthy();
  ...
  test('all locales display correctly', async ({ localePage }) => {
    await localePatterns.testPageInAllLocales(localePage.page, '/welcome', async (locale) => {
   ...
   */
  test('switching locales multiple times works correctly', async ({ localePage }) => {
    ...
    for (const [from, to] of switches) {
      await localePatterns.testLocaleSwitching(localePage.page, from, to, '/welcome');
    }
  });
  ...
  test('browser Accept-Language header is respected', async ({ browser }) => {
    ...
    const isFrench = url.includes('/fr/') || url.includes('/fr') || htmlLang === 'fr';
    expect(isFrench, `Expected French locale, got URL: ${url}, lang: ${htmlLang}`).toBeTruthy();
   ...
   */
  test('URL locale prefix takes priority over Accept-Language', async ({ page, context }) => {
   ...
   */
  test('NEXT_LOCALE cookie overrides Accept-Language', async ({ page, context }) => {
    // Set Accept-Language to Italian
    await context.setExtraHTTPHeaders({
      'Accept-Language': 'it-IT,it;q=0.9',
    });
   ...
   */
  test('locale is reflected in page HTML lang attribute', async ({ localePage }) => {
    await localePage.goto('/welcome');
