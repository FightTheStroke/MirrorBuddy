 ...
 */
function isTouchTargetValid(box: { width: number; height: number } | null): boolean {
  if (!box) return false;
...
test.describe('Parent Dashboard - Touch Targets (F-51)', () => {
  ...
  test('ALL buttons have minimum 44x44px touch targets', async ({ page }) => {
    ...
    if (invalidButtons.length > 0) {
      ...
      invalidButtons.forEach((btn) => {
        console.log(`  - "${btn.text}": ${btn.width}x${btn.height}px (need 44x44px)`);
      });
    }

    expect(invalidButtons, 'All buttons must have 44x44px minimum').toHaveLength(0);
  });
  ...
  test('ALL interactive links have adequate tap area', async ({ page }) => {
    ...
    if (invalidLinks.length > 0) {
      ...
      invalidLinks.forEach((link) => {
        console.log(`  - "${link.href}": ${link.width}x${link.height}px (need 44x44px)`);
      });
    }
  ...
  test('Filter controls are touch-friendly', async ({ page }) => {
    ...
    const filterControls = await page
      .locator("input[type='checkbox'], input[type='radio'], select, [role='listbox']")
      .all();
    ...
    for (const control of filterControls) {
      ...
      if (box) {
        ...
        if ((type === 'checkbox' || type === 'radio') && !isTouchTargetValid(box)) {
  ...
  test('Date pickers and similar controls are accessible', async ({ page }) => {
    ...
    if (invalidControls.length > 0) {
      console.log('\nSmall date controls:');
      invalidControls.forEach((ctrl) => {
        console.log(`  - ${ctrl.label}: ${ctrl.width}x${ctrl.height}px (need 44x44px or wrapper)`);
      });
    }
  });
  ...
  test('No critical overlapping touch targets', async ({ page }) => {
    ...
    expect(overlaps, 'No critical overlapping touch targets allowed').toHaveLength(0);
  });
  ...
  test('Expand/collapse buttons meet touch target size', async ({ page }) => {
    ...
    if (expandButtons.length > 0) {
      expect(invalidButtons, 'Expand/collapse buttons must have 44x44px minimum').toHaveLength(0);
    }
  });
  ...
  test('Icon buttons have minimum 44x44px size', async ({ page }) => {
    ...
    expect(invalidButtons, 'Icon buttons must have 44x44px minimum').toHaveLength(0);
  });
});
