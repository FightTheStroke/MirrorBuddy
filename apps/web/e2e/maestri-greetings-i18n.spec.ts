...
test.describe('Maestri Localized Greetings (i18n)', () => {
   ...
   */
  testAllLocales('should load maestri greetings in locale', async ({ localePage }) => {
   ...
   */
  test('formal maestri should use formal address in Italian locale', async ({ page }) => {
    ...
    for (const formalMaestro of FORMAL_MAESTRI) {
      ...
      const maestro = maestri.find(
        (m: { id: string }) =>
          m.id.toLowerCase().includes(formalMaestro.id) || m.id === formalMaestro.id,
      );
   ...
   */
  test('formal maestri should use formal address in French locale', async ({ page }) => {
   ...
   */
  test('informal maestri should use informal address in Italian locale', async ({ page }) => {
    const locale: Locale = 'it';
    const url = `/api/maestri?locale=${locale}`;
    const response = await page.request.get(url);
    const maestri = await response.json();
    ...
    for (const informalMaestro of INFORMAL_MAESTRI) {
      ...
      const maestro = maestri.find(
        (m: { id: string }) =>
          m.id.toLowerCase().includes(informalMaestro.id) || m.id === informalMaestro.id,
      );
   ...
   */
  test('same maestro should have different greetings in different locales', async ({ page }) => {
    ...
    for (const locale of locales) {
      ...
      const maestro = maestri.find(
        (m: { id: string }) => m.id.toLowerCase().includes(maestroId) || m.id === maestroId,
      );
  ...
  test('language teachers should show bilingual content', async ({ page }) => {
    ...
    for (const teacher of LANGUAGE_TEACHERS) {
      const maestro = maestri.find(
        (m: { id: string }) => m.id.toLowerCase().includes(teacher.id) || m.id === teacher.id,
      );
  ...
  test('English locale should show English greetings', async ({ page }) => {
    ...
    for (const maestro of sampleMaestri) {
      ...
      expect(greeting.match(/I'm|I am|Shall we|Let's|How can|help/i)).toBeTruthy();
   ...
   */
  test('greeting templates should be consistent in structure', async ({ page }) => {
   ...
   */
  test('Manzoni should use formal address in Italian and French', async ({ page }) => {
    ...
    // Italian test
    {
      ...
      const response = await page.request.get(url);
      const maestri = await response.json();
      const maestro = maestri.find((m: { id: string }) => m.id.toLowerCase().includes(maestroId));
   ...
   */
  test('Feynman should use informal address in Italian and French', async ({ page }) => {
   ...
   */
  testAllLocales('all maestri should have greetings in locale', async ({ localePage }) => {
    const locale = localePage.locale;
    const url = `/api/maestri?locale=${locale}`;
    const response = await localePage.page.request.get(url);
    const maestri = await response.json();
    ...
    const maestriWithoutGreeting = maestri.filter((m: { greeting?: string }) => !m.greeting);
   ...
   */
  test('maestri API should support locale query parameter', async ({ page }) => {
    const locales: Locale[] = ['it', 'en', 'es', 'fr', 'de'];
...
test.describe('Maestri Greetings - Edge Cases', () => {
   ...
   */
  test('maestro IDs should be normalized for greeting lookup', async ({ page }) => {
    const locale: Locale = 'it';
    const url = `/api/maestri?locale=${locale}`;
    const response = await page.request.get(url);
    const maestri = await response.json();
    ...
    const euclide = maestri.find((m: { id: string }) => m.id.toLowerCase().includes('euclide'));
   ...
   */
  test('greetings should not contain template placeholders', async ({ page }) => {
    const locales: Locale[] = ['it', 'en', 'es', 'fr', 'de'];
