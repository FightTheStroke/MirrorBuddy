...
test.describe('Locale Patterns', () => {
  test('should test page in all locales using pattern', async ({ page }) => {
    // Use a stable public page that exists for all locales.
    await localePatterns.testPageInAllLocales(page, '/welcome', async (locale) => {
  ...
  test('should test locale persistence across navigation', async ({ page }) => {
    await localePatterns.testLocalePersistence(page, 'it', ['/welcome', '/privacy', '/terms']);
  });

  test('should test locale switching', async ({ page }) => {
    await localePatterns.testLocaleSwitching(page, 'it', 'en', '/welcome');
  });
});
