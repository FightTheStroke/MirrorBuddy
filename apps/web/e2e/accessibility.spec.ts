...
test.describe('WCAG 2.1 AA Compliance', () => {
  for (const page of PAGES_TO_TEST) {
    test(`${page.name} (${page.path}) passes axe-core`, async ({ page: playwrightPage }) => {
...
test.describe('Keyboard Navigation', () => {
  ...
  test('skip link available for keyboard users', async ({ page }) => {
    ...
    if (!isSkipLink) {
      console.warn('No skip link found - consider adding one for keyboard users');
    }
  });
});
...
test.describe('Color and Contrast', () => {
  test('respects prefers-reduced-motion', async ({ page }) => {
    ...
    if (hasMotion) {
      console.warn('Some animations may still be active with prefers-reduced-motion');
    }
  });
});
...
test.describe('Instant Access - Floating Button', () => {
  ...
  test('floating button visible on legal pages', async ({ page }) => {
    const legalPages = [toLocalePath('/privacy'), toLocalePath('/terms'), toLocalePath('/cookies')];
...
test.describe('Instant Access - Quick Panel', () => {
  test('clicking button opens panel with 7 profiles', async ({ page }) => {
    ...
    const panel = page.locator('[role="dialog"]').filter({ hasText: 'Accessibilità' });
    await expect(panel).toBeVisible();

    // Check for all 7 profile presets
    const profiles = ['Dislessia', 'ADHD', 'Visivo', 'Motorio', 'Autismo', 'Uditivo', 'Motorio+'];

    for (const profile of profiles) {
      const profileBtn = page.locator(`button[aria-label="Attiva profilo ${profile}"]`);
...
test.describe('Instant Access - Profile Activation', () => {
  test('selecting dyslexia profile changes font', async ({ page }) => {
    ...
    test.skip(!!process.env.CI, 'Font loading unreliable in CI - needs font preloading');
  ...
  test('active profile shows indicator on floating button', async ({ page }) => {
    await page.goto(toLocalePath('/welcome'));
    await page.waitForLoadState('domcontentloaded');
...
test.describe('DSA Profile Support', () => {
  test('dyslexia font toggle works', async ({ page }) => {
    ...
    const body = page.locator('body');
    const initialFont = await body.evaluate((el) => window.getComputedStyle(el).fontFamily);
