...
test.describe('@tier-enforcement Trial Tier - Limits Enforcement', () => {
  ...
  test('trial tier prevents access to voice feature when limit reached', async ({ request }) => {
  ...
  test('trial tier shows upgrade prompt at 80% usage', async ({ trialPage }) => {
...
test.describe('@tier-enforcement Pro Tier - Feature Unlocks', () => {
  ...
  test('pro tier removes daily message limits', async ({ request }) => {
    ...
    if (response.ok()) {
      ...
      expect(tier.chatLimitDaily === null || tier.chatLimitDaily > 100).toBe(true);
    }
  });
  ...
  test('pro tier enables all features', async ({ request }) => {
    ...
    if (response.ok()) {
      ...
      const enabledCount = Object.values(tier.features).filter((v) => v === true).length;
  ...
  test('pro tier allows unlimited voice minutes', async ({ request }) => {
    ...
    if (response.ok()) {
      ...
      expect(tier.voiceMinutesDaily === null || tier.voiceMinutesDaily > 60).toBe(true);
    }
  });
});
...
test.describe('@tier-enforcement Locked Features - Pro Badge', () => {
  ...
  test('pdf export feature locked for trial tier', async ({ trialPage }) => {
    ...
    const exportButton = trialPage.locator("button:has-text('Export'), button:has-text('PDF')");
  ...
  test('mindmap feature shows upgrade prompt for trial', async ({ trialPage }) => {
    await trialPage.goto('/home');
    await trialPage.waitForLoadState('domcontentloaded');
    ...
    if (await mindmapButton.count()) {
      ...
      const upgradeModal = trialPage.locator("[data-testid='upgrade-modal'], text=/upgrade|pro/i");
...
test.describe('@tier-enforcement Tier Comparison & Feature Matrix', () => {
  test('tier comparison table displays all tiers', async ({ trialPage }) => {
    ...
    // Look for tier cards
    const tierCards = trialPage.locator("[data-testid='tier-card'], [data-testid='pricing-tier']");
  ...
  test('tier comparison shows feature differences', async ({ trialPage }) => {
    ...
    const featureRows = trialPage.locator("[data-testid='feature-row'], table tbody tr");
  ...
  test('upgrade button visible on trial/base tier cards', async ({ trialPage }) => {
    await trialPage.goto('/pricing');
    await trialPage.waitForLoadState('domcontentloaded');
  ...
  test('pro tier card shows all features enabled', async ({ trialPage }) => {
    ...
    if (await proCard.count()) {
      ...
      const disabledFeatures = proCard.locator("[data-testid='feature-disabled']");
...
test.describe('@tier-enforcement API Tier Enforcement', () => {
  test('API rejects feature access for insufficient tier', async ({ request }) => {
  ...
  test('API response includes usage telemetry', async ({ request }) => {
    ...
    if (response.ok()) {
      ...
      expect(data.usage || data.telemetry || data.headers || data.subscription).toBeDefined();
    }
  });
});
