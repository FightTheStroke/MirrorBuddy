...
test.describe('Analytics Page Grid - Very Small Mobile (xs < 375px)', () => {
  ...
  test('analytics - 1 column on very small screens', async ({ adminPage }) => {
    ...
    expect(bodyWidth, `Content wider than viewport: ${bodyWidth} > 320`).toBeLessThanOrEqual(330); // Allow small margin
  });

  test('analytics - cards are full width on very small screens', async ({ adminPage }) => {
    await adminPage.goto('/admin/analytics');
    await waitForPageReady(adminPage);
...
test.describe('Analytics Page Grid - Mobile (375px - 640px)', () => {
  ...
  test('analytics - no horizontal overflow on mobile', async ({ adminPage }) => {
    await adminPage.goto('/admin/analytics');
    await waitForPageReady(adminPage);
  ...
  test('analytics - cards have proper spacing on mobile', async ({ adminPage }) => {
    await adminPage.goto('/admin/analytics');
    await waitForPageReady(adminPage);
...
test.describe('Analytics Page Grid - Tablet (640px - 1024px)', () => {
  ...
  test('analytics - responsive columns on tablet', async ({ adminPage }) => {
    ...
    if (detailedCardsGrid) {
      ...
      const columnCount = (gridStyle.gridTemplateColumns.match(/\d+fr/g) || []).length;
      expect(columnCount).toBe(2);
    }
  });
...
test.describe('Analytics Page Grid - Desktop (1024px+)', () => {
  ...
  test('analytics - 2 columns on desktop lg breakpoint', async ({ adminPage }) => {
    await adminPage.goto('/admin/analytics');
    await waitForPageReady(adminPage);
