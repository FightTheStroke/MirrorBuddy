...
test.describe('Marketing Landing Page', () => {
  ...
  test('shows pricing section with 4 tiers', async ({ page }) => {
    ...
    const tierCards = page.locator("section[aria-labelledby='pricing-heading'] h3");
  ...
  test('FAQ accordion opens on click', async ({ page }) => {
    await page.goto('/en');
    const faqButton = page.locator('button').filter({ hasText: /safe for children/i });
  ...
  test('features grid shows 6 features', async ({ page }) => {
    await page.goto('/en');
    const featureCards = page.locator("section[aria-labelledby='features-heading'] h3");
