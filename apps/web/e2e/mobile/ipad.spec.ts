...
test.describe('iPad Mini Responsive UX', () => {
  ...
  test('sidebar should be persistent on tablet viewport', async ({ page, mobile }) => {
  ...
  test('voice panel should have appropriate width on tablet', async ({ page, mobile }) => {
    // Maestro buttons are in the main content area (not sidebar)
    const firstMaestro = page
      .locator('main button:has-text("Euclide"), main button:has-text("Galileo")')
      .first();
    if (await firstMaestro.isVisible()) {
      ...
      await voicePanel.waitFor({ state: 'visible', timeout: 5000 }).catch(() => {});
  ...
  test('touch targets should meet WCAG even with larger viewport', async ({ page, mobile }) => {
    const viewportWidth = await mobile.getViewportWidth();

    // Skip test on large viewports where hamburger menu is hidden (lg:hidden)
    if (viewportWidth >= 1024) {
      test.skip(true, 'Mobile sidebar overlay hidden on desktop viewports (lg:hidden)');
      return;
    }
    ...
    const toggleButton = page
      .locator('button[aria-label="Chiudi menu"], button[aria-label="Apri menu"]')
      .last();
  ...
  test('iPad should handle hover states appropriately', async ({ page, mobile }) => {
    const viewportWidth = await mobile.getViewportWidth();
    ...
    const hasHoverStyles =
      buttonClasses?.includes('hover:') || buttonClasses?.includes('group-hover');

    // Button should have hover styles defined (even if we can't trigger them in touch mode)
    expect(hasHoverStyles, 'Button should have hover styles for iPad with accessories').toBe(true);
  ...
  test('iPad should handle multitasking split view', async ({ page, mobile }) => {
  ...
  test('iPad should respect accessibility text size', async ({ page, mobile: _mobile }) => {
  ...
  test('iPad should support Pointer/Apple Pencil interactions', async ({ page, mobile }) => {
    const viewportWidth = await mobile.getViewportWidth();
    ...
    await expect(page.locator('button[aria-label="Apri menu"]').first()).toBeVisible({
      timeout: 3000,
    });
  });
});
