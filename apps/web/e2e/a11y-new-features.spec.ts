...
test.describe('Skip Link - WCAG 2.1 AA', () => {
  ...
  test('skip link has proper contrast and focus indicator', async ({ page }) => {
    await page.goto(toLocalePath('/'));
    await page.waitForLoadState('domcontentloaded');
...
test.describe('A11y Floating Button - ARIA Attributes', () => {
  ...
  test('floating button has aria-controls pointing to panel', async ({ page }) => {
    await page.goto(toLocalePath('/'));
    await page.waitForLoadState('domcontentloaded');
...
test.describe('A11y Features Integration', () => {
  test('skip link and floating button both work together', async ({ page }) => {
    ...
    const isSkipLink = activeElement.includes('#main') || activeElement.includes('#content');
  ...
  test('accessibility panel does not interfere with page content', async ({ page }) => {
    await page.goto(toLocalePath('/'));
    await page.waitForLoadState('domcontentloaded');
