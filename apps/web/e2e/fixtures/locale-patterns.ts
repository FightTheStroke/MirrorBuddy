...
export const localePatterns = {
  ...
  async testLocaleSwitching(
    ...
    if (!verification.isValid) {
      throw new Error(`Locale switch verification failed: ${verification.errors.join(', ')}`);
    }
  },
   ...
   */
  async testLocalePersistence(page: Page, locale: Locale, paths: string[]): Promise<void> {
    for (const path of paths) {
      ...
      const verification = await verifyPageLocale(page, locale);
      if (!verification.isValid) {
        throw new Error(`Locale persistence failed at ${path}: ${verification.errors.join(', ')}`);
      }
    }
  },
};
