...
test.describe('Memory System', () => {
  test('F-10: trial user gets basic memory without cross-maestro', async ({ trialPage: page }) => {
  ...
  test('F-10: pro user has full memory context access', async ({ adminPage: page }) => {
    await page.goto('/');
    await page.waitForLoadState('domcontentloaded');
    expect(page.url()).toContain('/');
  });

  test('F-10: pro user can toggle cross-maestro memory', async ({ adminPage: page }) => {
    await page.goto('/settings');
    await page.waitForLoadState('domcontentloaded');
  ...
  test('F-10: memory persists across page reload', async ({ trialPage: page }) => {
  ...
  test('F-10: memory state loads on subsequent visits', async ({ trialPage: page }) => {
