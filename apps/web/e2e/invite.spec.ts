...
test.describe('Beta Invite System', () => {
  test.describe('Beta Request Form', () => {
    test('should show validation errors for empty form', async ({ page }) => {
      ...
      const form = page.locator('form').filter({ hasText: 'Richiedi accesso Beta' });
    ...
    test('should submit valid beta request', async ({ page }) => {
      ...
      if (await form.isVisible()) {
        ...
        await form.locator('input[id="email"]').fill(`test-${Date.now()}@example.com`);
        await form
          .locator('textarea[id="motivation"]')
          .fill('I want to learn Italian with MirrorBuddy because it looks amazing!');
  ...
  test.describe('Admin Invites Page', () => {
    ...
    test('should have approve and reject buttons for pending invites', async ({ page }) => {
      await page.goto('/admin/invites');

      // Should show action buttons for pending invites (use first() as multiple may exist)
      await expect(page.locator("button:has-text('Approva')").first()).toBeVisible();
      await expect(page.locator("button:has-text('Rifiuta')").first()).toBeVisible();
    });
