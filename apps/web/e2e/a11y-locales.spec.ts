...
test.describe('WCAG 2.1 AA Compliance - All Locales', () => {
   ...
   */
  testAllLocales('all pages pass axe-core WCAG 2.1 AA', async ({ localePage }) => {
...
test.describe('Locale-Specific Accessibility Features', () => {
   ...
   */
  testAllLocales('locale has correct language direction', async ({ localePage }) => {
    await localePage.goto('/');
    await localePage.page.waitForLoadState(WAIT_FOR_NETWORK_IDLE);
   ...
   */
  testAllLocales('text is readable in current locale', async ({ localePage }) => {
    await localePage.goto('/welcome');
    await localePage.page.waitForLoadState(WAIT_FOR_NETWORK_IDLE);
...
test.describe('Color Contrast - All Locales', () => {
   ...
   */
  testAllLocales('homepage has sufficient color contrast', async ({ localePage }) => {
    await localePage.goto('/');
...
test.describe('Heading Hierarchy - All Locales', () => {
   ...
   */
  testAllLocales('homepage has proper heading hierarchy', async ({ localePage }) => {
    await localePage.goto('/');
    await localePage.page.waitForLoadState(WAIT_FOR_NETWORK_IDLE);
...
test.describe('Form Accessibility - All Locales', () => {
  ...
  testAllLocales('forms have accessible labels', async ({ localePage }) => {
    ...
    const results = await new AxeBuilder({ page: localePage.page })
      .withRules(['label', 'form-field-multiple-labels', 'label-content-name-mismatch'])
      .analyze();
...
test.describe('Image Accessibility - All Locales', () => {
   ...
   */
  testAllLocales('images have alt text or are marked as decorative', async ({ localePage }) => {
    await localePage.goto('/');
    await localePage.page.waitForLoadState(WAIT_FOR_NETWORK_IDLE);
    ...
    if (results.violations.length > 0) {
      ...
      for (const violation of results.violations) {
        console.log(`Found ${violation.nodes.length} images without alt text`);
      }
    }
...
test.describe('Comprehensive Accessibility Audit by Locale', () => {
  ...
  for (const locale of SUPPORTED_LOCALES) {
    test(`[${locale}] comprehensive WCAG 2.1 AA audit`, async ({ page, context }) => {
