...
test.describe('OAuth Flow - Google Integration', () => {
  ...
  test('GET /api/auth/google - requires userId parameter', async ({ request }) => {
  ...
  test('GET /api/auth/google/status - returns disconnected for new user', async ({ request }) => {
  ...
  test('GET /api/auth/google/callback - validates state parameter', async ({ request }) => {
  ...
  test('GET /api/auth/google/callback - rejects invalid state', async ({ request }) => {
    const response = await request.get('/api/auth/google/callback?code=fake&state=invalid-state', {
      maxRedirects: 0,
    });
  ...
  test('GET /api/auth/google/callback - handles OAuth errors from Google', async ({ request }) => {
...
test.describe('State Parameter Validation', () => {
  test('OAuth state includes userId and is encoded', async ({ request }) => {
    ...
    for (const state of invalidStates) {
      const response = await request.get(`/api/auth/google/callback?code=test&state=${state}`, {
        maxRedirects: 0,
      });
...
test.describe('Session Security', () => {
  test('Authenticated endpoints reject unsigned cookies (F-07)', async ({ request }) => {
...
test.describe('COPPA Compliance (F-09)', () => {
  test('GET /api/coppa - returns status for authenticated user', async ({ request }) => {
    // Ensure user exists
