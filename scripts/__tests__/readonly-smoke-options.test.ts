...
describe('readonly smoke command boundary (pure validation, no database)', () => {
  it('accepts only the explicit synthetic target and fixed command shape', () => {
    expect(readonlySmokeOptions(args, localEnv, '20.20.2')).toEqual({
      action: 'issue',
      target: 'synthetic',
      directory,
      temporaryRoot: tmpdir(),
    });
    expect(readonlySmokeOptions(['revoke', ...args.slice(1)], localEnv, '20.20.2').action).toBe(
      'revoke',
    );
  });
  ...
  it.each([
    ...
    expect(() => readonlySmokeOptions(input, localEnv, '20.20.2')).toThrow();
  });
  it.each<Partial<NodeJS.ProcessEnv>>([
    ...
    expect(() => readonlySmokeOptions(args, { ...localEnv, ...change }, '20.20.2')).toThrow();
  });
  it.each<Partial<NodeJS.ProcessEnv>>([
    { GITHUB_ACTIONS: '' },
