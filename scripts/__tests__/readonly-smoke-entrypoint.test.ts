...
// Every case below already bounds its spawnSync at 20_000ms; the test-level
// timeout must sit just above that budget, not at Vitest's 5_000ms default.
const ENTRYPOINT_TEST_TIMEOUT_MS = 25_000;

describe('actual conditioned readonly CLI entrypoint, hermetic rejection only', () => {
  it(
    'runs through the existing launcher and fails before DB imports on missing arguments',
    () => {
  ...
  it(
    'rejects production authority before importing or connecting to any database',
    () => {
  ...
  it(
    'refuses bare tsx without the react-server condition',
    () => {
      const result = spawnSync(
        process.execPath,
