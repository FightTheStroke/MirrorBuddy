...
describe('real producer and consumer processes with synthetic external build/deployment boundaries', () => {
  it('only promotes after consuming the artifact actually produced after validation and build', () => {
  ...
  it('rejects a success-shaped READY response when build execution was skipped entirely', () => {
    const result = consume();
  ...
  it.each(['preview', 'development'])('does not produce production proof in %s', (target) => {
    seedOldOutput();
  ...
  it('preserves a failed build exit and never emits proof', () => {
    seedOldOutput();
  ...
  it('requires a freshly generated Next BUILD_ID even after build command exit zero', () => {
    seedOldOutput();
  ...
  it('propagates receipt generation filesystem failure', () => {
...
`,
  ...
  it('propagates external build termination rather than producing proof', () => {
    producer.executable('npm', `process.kill(process.pid, 'SIGTERM');`);
    expect(producer.invoke(productionValues()).status).toBe(1);
    expect(receiptExists()).toBe(false);
  });

  it('blocks validator mutation during the external build', () => {
    producer.executable(
      'npm',
      `
const fs = require('node:fs');
...
`,
  ...
  it.each([
    ...
    { nextBuildId: '' },
    { extra: 'unqualified' },
  ])('rejects altered or replayed actual receipt %j', (delta) => {
