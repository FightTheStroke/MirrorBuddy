...
async function extractCertificate() {
  ...
  try {
    ...
    const connection = 'connection' in client ? client.connection : undefined;
    const socket =
      connection && typeof connection === 'object' && 'stream' in connection
        ? connection.stream
        : undefined;
    ...
    if (socket instanceof TLSSocket) {
      ...
      if (cert && Object.keys(cert).length > 0) {
        ...
        console.log(`  Issuer: ${cert.issuerCertificate?.subject?.CN || 'unknown'}`);
        ...
        if (cert.issuerCertificate && cert.issuerCertificate !== cert) {
          ...
          console.log(`  Issuer: ${intermediate.issuerCertificate?.subject?.CN || 'unknown'}`);
          ...
          if (intermediate.issuerCertificate && intermediate.issuerCertificate !== intermediate) {
            const root = intermediate.issuerCertificate;
            console.log('\nRoot CA Certificate:');
            console.log(`  Subject: ${root.subject.CN}`);
            console.log(`  Issuer: ${root.issuer.CN}`);

            const isSelfSigned = JSON.stringify(root.subject) === JSON.stringify(root.issuer);
          ...
          // Save to file
          const outputPath = path.join(process.cwd(), 'config', 'supabase-chain.pem');
          fs.mkdirSync(path.dirname(outputPath), { recursive: true });
          fs.writeFileSync(outputPath, certChain, 'utf-8');

          const certCount = (certChain.match(/BEGIN CERTIFICATE/g) || []).length;
          console.log(`\n✓ Certificate chain saved to: ${outputPath}`);
          console.log(`  Certificates in chain: ${certCount}`);
          console.log('\nNext steps:');
          console.log('  1. Update src/lib/db.ts to load from config/supabase-chain.pem');
