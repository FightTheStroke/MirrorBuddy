...
interface GrafanaAlertRuleResponse {
  uid?: string;
  title?: string;
  id?: number;
  orgID?: number;
...
const GRAFANA_URL =
  process.env.GRAFANA_URL || "https://mirrorbuddy.grafana.net";
...
if (!GRAFANA_API_KEY) {
  ...
  console.error(
    "  3. Set environment variable: export GRAFANA_API_KEY=<your-key>",
  );
  process.exit(1);
}
...
async function createAlertRule(rule: AlertRuleConfig): Promise<void> {
  ...
  try {
    ...
    const data = (await response.json()) as GrafanaAlertRuleResponse;
...
async function main(): Promise<void> {
  ...
  try {
    ...
    console.log(
      "  1. Verify alerts in Grafana: https://mirrorbuddy.grafana.net/alerting/list",
    );
