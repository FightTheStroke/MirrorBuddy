...
REQUIRED_VARS=(
	...
	"ADMIN_EMAIL" "ADMIN_PASSWORD" "ADMIN_READONLY_EMAIL" "ADMIN_READONLY_COOKIE_VALUE"
	"CRON_SECRET"
	...
	"GRAFANA_CLOUD_API_KEY" "GRAFANA_CLOUD_PUSH_INTERVAL" "GRAFANA_SERVICE_ACCOUNT_TOKEN"
	# Stripe
	"STRIPE_SECRET_KEY" "STRIPE_WEBHOOK_SECRET" "NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY"
	# Misc
	"PROTECTED_USERS" "TRIAL_BUDGET_LIMIT_EUR" "BRAVE_SEARCH_API_KEY"
	"ADMIN_READONLY_EMAIL" "ADMIN_READONLY_COOKIE_VALUE"
	# Public URLs
	...
	# Chat deployment names (GA only — the gpt-5.x-chat/edu deployments were
	# dropped on 2026-09-16: they are backed by a preview model retiring 2026-10-05)
...
# Dev-only / optional vars to skip (must match fix-vercel-env-vars.sh SKIP_VARS).
# AZURE_OPENAI_REALTIME_DEPLOYMENT_V21 is optional by design (ADR 0169): token routes
# gracefully fall back to V2/V15/legacy when unset, so it must NOT be a required Vercel var.
SKIP_PATTERNS="DEV_DATABASE_URL|TEST_DATABASE_URL|TEST_DIRECT_URL|PROD_TEST_USER_ID|PROD_TEST_USER_EMAIL|PROD_TEST_USER_USERNAME|PROD_TEST_USER_PASSWORD|PROD_TEST_USER_COOKIE_VALUE|PROD_TEST_ADMIN_ID|PROD_TEST_ADMIN_EMAIL|PROD_TEST_ADMIN_USERNAME|PROD_TEST_ADMIN_PASSWORD|OLLAMA_URL|OLLAMA_MODEL|NODE_TLS_REJECT_UNAUTHORIZED|VERCEL_TOKEN|VERCEL_TEAM_ID|APPLE_ID|TEAM_ID|ITC_TEAM_ID|FASTLANE_USER|MATCH_GIT_URL|MATCH_PASSWORD|POSTGRES_DATABASE|POSTGRES_HOST|POSTGRES_PASSWORD|POSTGRES_USER|SUPABASE_JWT_SECRET|SUPABASE_PUBLISHABLE_KEY|SUPABASE_SECRET_KEY|NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY|AZURE_OPENAI_REALTIME_DEPLOYMENT_V21"
