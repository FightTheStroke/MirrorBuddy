...
import { Pool } from 'pg';
import { getSSLConfig } from '../apps/web/prisma/ssl-config';
...
async function main() {
  ...
  const orphans = activeUsers24h.filter((a) => !registeredIds.has(a.identifier));
  ...
  orphans.forEach((o, i) => {
    console.log(`${i + 1}. ${o.identifier} (${o._count.identifier} activity records)`);
  });
  ...
  for (const orphan of orphans.slice(0, 3)) {
    ...
    activities.forEach((a) => {
      console.log(`  ${a.timestamp.toISOString()} - ${a.userType} - ${a.route}`);
    });
  }
