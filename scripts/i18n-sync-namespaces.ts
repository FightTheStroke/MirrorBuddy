...
function saveNamespaceFile(locale: string, namespace: string, data: unknown): void {
  const filePath = path.join(MESSAGES_DIR, locale, `${namespace}.json`);
  fs.writeFileSync(filePath, JSON.stringify(data, null, 2) + '\n');
}

function syncNamespace(refData: unknown, targetData: unknown, missingKeys: string[]): unknown {
  const result =
  ...
  for (const key of missingKeys) {
    ...
    const value = typeof refValue === 'string' ? `[TRANSLATE] ${refValue}` : refValue;
...
function removeExtraKeys(targetData: unknown, extraKeys: string[]): unknown {
  const result =
    typeof targetData === 'object' && targetData !== null
      ? JSON.parse(JSON.stringify(targetData))
      : {};
  ...
  for (const key of extraKeys) {
    deleteValueByPath(result as Record<string, unknown>, key);
  }
...
function main() {
  ...
  if (!quiet) {
    ...
    else if (addMissing) mode = 'ADD MISSING';
    else if (removeExtra) mode = 'REMOVE EXTRA';
  ...
  for (const namespace of NAMESPACES) {
    ...
    if (!refData) {
      if (!quiet) console.warn(`⚠️  Reference file missing: ${REFERENCE_LOCALE}/${namespace}.json`);
      continue;
    }
    ...
    for (const locale of OTHER_LOCALES) {
      ...
      if (!targetData) {
        if (!quiet) console.log(`❌ ${locale}/${namespace}.json - FILE MISSING`);
        if (addMissing && !dryRun) {
          ...
          console.log(`   ✅ Created with ${getAllKeys(refData).length} keys (marked [TRANSLATE])`);
        }
        continue;
      }
      ...
      if (report.missingKeys.length > 0) {
        if (!quiet) {
          ...
          report.missingKeys.slice(0, 5).forEach((k) => console.log(`     - ${k}`));
        ...
        if (addMissing && !dryRun) {
          ...
          if (!quiet) console.log(`   ✅ Added missing keys (marked [TRANSLATE])`);
        }
      }

      if (report.extraKeys.length > 0) {
        if (!quiet) {
          console.log(`   Extra ${report.extraKeys.length} keys (not in reference):`);
        ...
        if (removeExtra && !dryRun) {
          targetData = removeExtraKeys(targetData, report.extraKeys);
          saveNamespaceFile(locale, namespace, targetData);
          if (!quiet) console.log(`   ✅ Removed extra keys`);
        }
      }
    }
  ...
  if (quiet) {
    ...
    console.log(`${status} i18n sync: ${totalMissing} missing, ${totalExtra} extra`);
  } else {
    console.log('📊 Summary:');
    ...
    if (totalExtra > 0 && !removeExtra) {
      console.log('💡 Run with --remove-extra to remove extra keys not in reference.');
    }
    if (dryRun) {
