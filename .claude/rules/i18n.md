...
## MANDATORY: All UI Text Must Be Internationalized

**Every new feature MUST support all 5 languages from day 1.**

| Rule                     | Enforcement                                      |
| ------------------------ | ------------------------------------------------ |
| No hardcoded text in JSX | ESLint rule `no-hardcoded-italian`               |
| No kebab-case keys       | ESLint rule `no-kebab-case-i18n-keys` (ADR 0091) |
| All 5 languages synced   | Pre-commit hook runs `npm run i18n:check`        |
| camelCase keys only      | Auto-fixed by `npm run lint -- --fix`            |

### Before Writing ANY UI Component

1. **Identify all user-visible strings** (buttons, labels, messages, errors)
2. **Add keys to `messages/it/*.json` first** (Italian is the reference)
3. **Run `npm run i18n:sync`** to propagate to other languages
4. **Use `t()` calls** instead of hardcoded text

### Key Naming Convention (ADR 0091)
...
// WRONG - kebab-case (blocked by ESLint)
t("with-teachers"); // ❌ ESLint error
t("any-ability"); // ❌ ESLint error
```
