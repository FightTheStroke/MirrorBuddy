 ...
 */
export function parseAcceptLanguageHeader(header: string | null | undefined): Locale {
  ...
  try {
    ...
    const preferences = header
      ...
      .map((pref) => {
        ...
        const quality = parseFloat(parts.find((p) => p.startsWith('q='))?.substring(2) || '1');
...
export function detectLocaleFromRequest(options: {
  ...
  if (options.acceptLanguageHeader) {
    const parsedLocale = parseAcceptLanguageHeader(options.acceptLanguageHeader);
    if (parsedLocale !== defaultLocale || isValidLocale(options.acceptLanguageHeader)) {
