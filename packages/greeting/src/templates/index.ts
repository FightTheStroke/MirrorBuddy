...
import type { SupportedLanguage } from '@mirrorbuddy/types';
...
export const BILINGUAL_GREETINGS: Record<string, Record<SupportedLanguage, string>> = {
  shakespeare: {
 ...
 */
export function applyGreetingTemplate(template: string, variables: Record<string, string>): string {
...
/**
 * Resolve a greeting template by language with Italian fallback.
 * Used by maestri/coaches so static fallback is only used when template is missing.
 */
export function getGreeting(
  ...
  return templates[language] || templates.it || fallbackTemplate;
}
