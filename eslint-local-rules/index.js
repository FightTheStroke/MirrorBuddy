...
const noI18nInProviders = {
  meta: {
    type: 'problem',
    docs: {
      description: 'Prevent next-intl hooks in files outside LocaleProvider context',
  ...
  create(context) {
    ...
    return {
      ...
      CallExpression(node) {
        if (node.callee.type === 'Identifier' && forbiddenHooks.includes(node.callee.name)) {
          context.report({
            node,
            messageId: 'noI18nInProviders',
          });
...
const preferValidateAuth = {
  meta: {
    type: 'suggestion',
    docs: {
      description: 'Prefer validateAuth() over direct AUTH_COOKIE_NAME reads in API routes',
...
const noHardcodedItalian = {
  meta: {
    type: 'suggestion',
    docs: {
      description: 'Detect hardcoded Italian strings in JSX and suggest using i18n',
      category: 'Best Practices',
      recommended: true,
